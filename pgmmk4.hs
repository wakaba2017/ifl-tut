...
data Node
  ...
  | NLAp Addr Addr Int PgmPendingList
                       -- Locked Applications  -- PGM Mark2で追加、PGM Mark4で変更
  | NLGlobal Int GmCode Int PgmPendingList
                       -- Locked Globals       -- PGM Mark2で追加、PGM Mark4で変更
  deriving Show  -- テキストにはないけれど追加
instance Eq Node
  where
    ...
    NLAp     a b e g == NLAp     c d f h = False  -- not needed  -- PGM Mark2で追加、PGM Mark4で変更
    NLGlobal a b e g == NLGlobal c d f h = False  -- not needed  -- PGM Mark2で追加、PGM Mark4で変更
...
doAdmin ((out, heap, globals, sparks, unUsedIdNumList, stats), local)
  = ((out, heap, globals, sparks, unUsedIdNumList, stats'), local')
    where newLocal = [l |l <- local, l /= emptyTask]
          (local', stats') = foldr filter ([], stats) newLocal
          filter :: PgmLocalState -> ([PgmLocalState], GmStats) -> ([PgmLocalState], GmStats)
...
getArg :: Node -> Addr
getArg (NAp a1 a2) = a2
getArg (NLAp a1 a2 id pl) = a2  -- PGM Mark2で追加、PGM Mark4で変更
    ...
    where
      ...
      newState (NLGlobal 0 c id pl)
        = (fst state_, emptyTask)  -- PGM Mark2で追加、PGM Mark4で変更
          where newPl = (snd (putCode [Unwind] state)) : pl
                newHeap = hUpdate heap a (NLGlobal 0 c id newPl)
                state_ = putHeap newHeap state
...
par state = putSparks (newTask : t) $ putStack s newState
            where (a : s) = getStack state
                  ...
                  (pidnum : newUnUsedIdNumList) = getUnUsedIdNumList state
                  newState = putUnUsedIdNumList newUnUsedIdNumList state
                  newTask = makeTask ((a, idnum), pidnum)
...
unlockAll addr state = unlockArg addr (unlock addr state)
  where
    unlockArg addr state
      = case hLookup heap addr of
        NAp _ a2        -> unlockAll a2 state
        NLAp _ a2 id pl -> unlockAll a2 state
        _               -> state
        where
          heap = getHeap state
...
showNode s a (NLGlobal n g id pl)
  = iConcat [iStr "Locked Global (", iNum id, iStr ") ", iStr v]
    where v = hd [n | (n,b) <- getGlobals s, a==b]  -- PGM Mark2で追加、PGM Mark4で変更
showNode s a (NLAp a1 a2 id pl)
  = iConcat [iStr "Locked Ap (", iNum id, iStr ") ", iStr (showaddr a1),
             iStr " ", iStr (showaddr a2)]  -- PGM Mark2で追加、PGM Mark4で変更
...
showContent s a (NLGlobal n g id pl)
  = iConcat [iStr "L", iNum id, iStr "(",
             iStr (hd [n | (n,b) <- getGlobals s, a==b]),
             iStr ")"]
showContent s a (NLAp a1 a2 id pl)
  = iConcat [iStr "L", iNum id, iStr "(",
             showContent s a1 (hLookup (getHeap s) a1),
             iStr " ",
             showContent s a2 (hLookup (getHeap s) a2),
...
showSparks sparks
  = iConcat [iStr " Sparks:[",
             ...
             iStr "]"]
    where
      subFuncForShowSparks (idnum, _, _, _, _, _)
        = iConcat [iStr "Task ", iNum (fst idnum)]
