...
instance Eq Instruction
  where
    Unwind       == Unwind       = True    -- Unwind 同士の比較結果は True
    Pushglobal a == Pushglobal b = a == b  -- Pushglocal a と Pushglobal b の比較結果は a と b の比較結果と等しい
    Pushint    a == Pushint    b = a == b  -- Pushint a と Pushint b の比較結果は a と b の比較結果と等しい
    Push       a == Push       b = a == b  -- Push a と Push b の比較結果は a と b の比較結果と等しい
    Mkap         == Mkap         = True    -- Mkap 同士の比較結果はTrue
    Slide      a == Slide      b = a == b  -- Slide a と Slide b の比較結果は a と b の比較結果と等しい
    _            == _            = False   -- 上記以外の比較結果は False
...
pushint :: Int -> GmState -> GmState  -- 遷移規則 (3.6)
pushint n state
