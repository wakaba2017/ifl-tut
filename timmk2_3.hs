...
compileR (EAp (EAp (EAp (EVar "if") e0) e1) e2) env  -- Mark2で追加
  = compileB e0 env [Cond e1' e2']
    where e1' = compileR e1 env
          e2' = compileR e2 env
compileR (EAp (EAp (EVar op) e1) e2) env  -- Mark2で追加
  | op `elem` op_list = compileB (EAp (EAp (EVar op) e1) e2) env [Return]
  | otherwise         = Push (compileA e2 env) : Push (compileA e1 env) : [Enter (compileA (EVar op) env)]
  where op_list = ["+", "-", "*", "/", "<", "<=", ">", ">=", "==", "~="]
compileR (EAp (EVar "negate") e) env  -- Mark2で追加
  = compileB (EAp (EVar "negate") e) env [Return]
...
compileR (ENum n)    env = compileB (ENum n) env [Return]  -- Mark2で変更
...
-- Mark2で追加
compileB :: CoreExpr -> TimCompilerEnv -> [Instruction] -> [Instruction]  -- Bスキーム
compileB (EAp (EAp (EVar op) e1) e2) env cont
  = compileB e2 env (compileB e1 env (i : cont))
    where
      i = case op of
          "+"  -> Op Add
          "-"  -> Op Sub
          ...
          "==" -> Op Eq
          "~=" -> Op NotEq
compileB (EAp (EVar "negate") e) env cont
  = compileB e env (Op Neg : cont)
compileB (ENum n) env cont
  = PushV (IntVConst n) : cont
compileB e env cont
  = Push (Code cont) : compileR e env
---------------------------------------
-- プログラムのコンパイル (ここまで) --
---------------------------------------
