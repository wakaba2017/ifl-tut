...
\subsection{Full-lazy lambda lifting in the presence of \texttt{let(rec)}s}
\subsection{Full laziness without lambda lifting}
\subsection{A full lazy lambda lifter}
\subsection{Separating the lambdas}
\subsection{Adding level numbers}
\subsection{Identifying MFEs}
\subsection{Renaming variables}
\subsection{Floating \texttt{let(rec)} expressions}
\newpage

\section{Mark 5: Improvements to full laziness}
\subsection{Adding \texttt{case} expressions}
\subsection{Eliminating redundant supercombinators}
\subsection{Avoiding redundant full laziness}
\newpage

\section{Mark 6: Dependency analysis \dag}
...
Bird の目的は、初期仕様を連続的に変換することによって、効率的な完全遅延ラムダリフターを形式的に開発することです。
