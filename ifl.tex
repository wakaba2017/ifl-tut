...
図 (f) では、\texttt{Task 1} がスパインを巻き戻し始め、\texttt{Task 2} が更新を実行していることがわかります。
\vskip\baselineskip
...
\begin{verbatim}
  initialTask :: Addr -> PgmLoccalState
...
巻き戻し時に、アプリケーションノードとスーパーコンビネータを引数なしでロックする必要があります。
...
ロックする必要があるノードは、アプリケーションノードと引数のないグローバルノードです。
...
ロックされたアプリケーションノードを \texttt{unlock} する場合、そのノードが指すスパインもロック解除されていることを確認する必要があります。
...
これらの遷移は現在定義されています。
遷移規則 5.2 では、アプリケーションノード(\texttt{*NAp} で表される)をロックすること以外は、\texttt{Unwind} は Mark 1 マシンと同じ遷移規則を持つことがわかります。
\vskip\baselineskip
