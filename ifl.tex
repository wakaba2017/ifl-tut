...
コア言語は可能な限り小さく設計されているため、実装は簡単ですが、効率を損なうことなく最新の非厳密な関数型言語をコア言語に変換できるほどリッチです。
...
\begin{itemize}
	...
	\item 第4章では、評価モデルがGマシンの評価モデルとは大きく異なる抽象マシンである\textbf{Three Instruction Machine}(TIM)についても同じ演習を繰り返します。
...
\textbf{完全な遅延評価}は、以前はラムダリフティングと切り離せないと見なされていた関数型プログラムの特性です。
第6章では、それらが実際にはまったく異なることを示し、ラムダリフティングとは別のパスで完全な遅延評価を実装する方法を示します。
\vskip\baselineskip
...
(関数型プログラミングの適切な紹介には、[Bird and Wadler 1988] および [Holyer 1991] が含まれます。)
...
\section*{「The implementation of functional programming languages」との関係}
...
まず、コア言語の非形式的な紹介から始めます(セクション\ref{sec_1_1})。
これに続いて、次のようにしてコア言語をより形式的に定義します。
...
プログラマは、\texttt{quadruple}で2つの加算だけが実行されることを合理的に期待できます。
\vskip\baselineskip
...
\subsection{構造化データ型} \label{sec_1_1_3}
...
\subsection{コンストラクタの表現} \label{sec_1_1_4}
...
\section{コア言語のデータ型} \label{sec_1_3}
...
つまり、\texttt{let(rec)} 定義の左辺、またはラムダ抽象化です。
...
\section{A small standard prelude} \label{sec_1_4}
\newpage

\section{コア言語用のプリティプリンタ} \label{sec_1_5}

\texttt{CoreProgram} 型の値を取得したら、それを表示できると便利なことがよくあります。
Haskell の組み込み機能は、ここではあまり役に立ちません。
たとえば、Haskell プロンプトに応答して \texttt{preludeDefs} と入力すると、生成される出力を理解するのはかなり困難です。
(それを試してみてください。)
\vskip\baselineskip
...
次に、\texttt{pprint preludeDefs} と入力して、表示すると \texttt{preludeDefs} の適切にフォーマットされたバージョンのように見える文字のリストを取得することを期待できます。
このセクションの目標は、そのような関数を作成することです。
\vskip\baselineskip

プログラムの結果がリストの場合、Haskell は通常、リスト項目をコンマで区切り、角かっこで囲んで印刷します。
ただし、プログラムの結果が \texttt{[Char]} 型の特殊なケースでは、Haskell は角かっことコンマを付けずに「すべて押しつぶされた」リストを表示します。
...
内側の \texttt{++} は \texttt{length xs1} に比例する時間がかかりますが、
外側の \texttt{++} は \texttt{xs1 ++ xs2} の長さに比例する時間がかかるため、合計時間は (2 * \texttt{length xs1}）+ \texttt{length xs2} になります。
一般に、このネストされた append にさらにリストを追加した場合、コストは結果の長さの2乗になる可能性があります！
もちろん、式を逆に括弧でくくった場合、コストは結果の長さに比例しますが、残念ながら、プリティプリンタでこれを保証することはできません。
\vskip\baselineskip
...
初期のアキュムレータ $acc$ は \texttt{e1} であり、結合関数は \texttt{EAp} コンストラクタです。
...
\begin{description}
	\item[演習 1.1] \texttt{n} のさまざまな値に対して、計算に必要な Miranda のステップ数を測定します。
		...
		実行コストが \texttt{n} でどのように上昇するかを示すグラフをスケッチし、\texttt{n} に関してほぼ2乗であることを確認します。
\end{description}

\subsection{プリティプリンティングのための抽象データ型}

コストが表示されるプログラムのサイズの2倍になるプリティプリンタは、明らかに容認できないので、それを回避する方法を見つけたほうがよいでしょう。
\vskip\baselineskip

この問題を2つの部分に分けることができます: \textbf{「実行したい操作は?」}と\textbf{「それらを実行する効率的な方法は?」}です。
...
\texttt{++} を \texttt{iAppend} \footnote{Haskell では、識別子の前後にバッククォート記号を書くと、それが中置演算子に変わり、
...
定義をより読みやすくするために、\texttt{iConcat} と \texttt{iInterleave} の2つの新しい関数を使用しました。
...
\begin{description}
	\item[演習 1.2] \texttt{iAppend} と \texttt{iNil} に基づいて \texttt{iConcat} と \texttt{iInterleave} を定義します。
\end{description}

一般に、すべてのプリティプリンティング関数は \texttt{Iseq} を返し、
表示したいもの全体を表す \texttt{Iseq} に \texttt{iDisplay} をトップレベルで1回だけ適用します。

\begin{description}
	\item[演習 1.3] \texttt{pprExpr} にさらに式を追加して \texttt{case} 式とラムダ式を処理し、
		\texttt{pprAExpr} と \texttt{pprProgram} の定義を同じスタイルで記述します。
\end{description}

\subsection{\texttt{iseq}の実装}
...
操作 \texttt{iNil}、\texttt{iStr}、\texttt{iAppend} はすべて、関連するコンストラクタを使用するだけです。
...
私たちのプリティプリンタがそのような関数適用を認識し、それらを中置形式で表示できれば素晴らしいと思います。
...
この式はまだあまりうまく機能していません。括弧が多すぎるからです。
次の式
...
\subsection{\texttt{iseq} のその他の便利な機能}
...
このセクションから導き出したい2つの一般的なポイントがあります。
...
\section{コア言語のパーサ} \label{sec_1_6}
...
この文法が、\texttt{|} と \texttt{\&} は右結合的であるのに対し、関係演算子は非結合的であるという事実をどのように表現しているかに注目してください。
これほど多くの規則を記述するのはかなり面倒ですが、これは最初から意図していたことを明示的に示しているだけです。
しかしここで2つ目の問題が発生します。
これらの規則から直接実装されたパーサーは、恐ろしく非効率になってしまうのです！
$expr1$ の生成規則を考えてみましょう。
単純なパーサーは、$expr2$ を認識しようとし、次にバーチカルバー \texttt{|} を探します。
もしバーチカルバー \texttt{|} が見つからなかった場合(よくあることですが)、$expr2$ を再び探すために、元の入力を苦労して再解析することになります。
さらに悪いことに、$expr2$ を解析するたびに、$expr3$ を解析する試行が2回必要になり、その結果、$expr4$ を解析する試行が4回必要になるなど、状況は続きます。
\vskip\baselineskip

2つの生成規則の間で $expr2$ の解析を共有したいのですが、これは $expr1$ 生成規則を2つに分割することで簡単に実現できます。
...
ここで、$\epsilon$ は空文字列を表します。
$expr1$ の生成規則では、$expr1$ はバーチカルバー \texttt{|} の後に $expr1$ が続くか、空であるかのいずれかであるとされています。
もうすぐです！
最後の質問は、$expr1$ のパーサの型は何か、ということです。
\texttt{|} $expr1$ という句は式の一部に過ぎず、空文字列 $\epsilon$ も式ではないため、型 \texttt{parser coreExpr} にはなり得ません。
いつものように、文法の変換によって構造が破壊されています。
\vskip\baselineskip
...
\subsection{次のredexを見つけるためのスパインのアンワインド}
...
\begin{enumerate}
	\item スーパーコンビネータまたは組み込みプリミティブに到達するまで、ルートから開始して、関数適用ノードの左側の枝をたどります。
	      この関数適用ノードの左分岐チェーンはエクスプレッションの\textbf{スパイン}と呼ばれ、このプロセスはスパインの\textbf{アンワインド}と呼ばれます。
...
\vskip\baselineskip
\texttt{f} が2つではなく4つの引数をとった場合のように、評価の結果が部分適用である可能性がある場合は、
上記の手順2の前に、スパインに十分な関数適用ノードがあることを確認する必要があります。
そうでない場合、式はWeak Head Normal Form (WHNF)に達しています。
サブ式\texttt{E1}、\texttt{E2}、および\texttt{E3}にはまだ redexe が含まれている可能性がありますが、
...
\subsection{定作用形式 \dag}

一部のスーパーコンビネータには引数がありません。それらは、定作用形式または CAF と呼ばれます。
たとえば、以下の \texttt{fac20} は CAF です。

\begin{verbatim}
        fac20 = factorial 20
\end{verbatim}

CAF の興味深い点は、スーパーコンビネータ自体が redex であるということです。
\texttt{fac20} が呼び出されるたびに、\texttt{factorial 20} の新しいコピーをインスタンス化する必要はありません。
これは、\texttt{factorial 20} の計算を繰り返すことを意味するためです。
...
\section{Mark 2: \texttt{let(rec)} 式}
...
テンプレートインスタンス化マシンと同じように、スパインがほどかれています。
...
\section{テンプレートを構築するためのコード シーケンス}

テンプレートインスタンス化マシンは次のように動作することを思い出してください。

\begin{itemize}
	\item スタックの一番上にある単一のアイテムが整数へのポインタである場合、マシンは終了しました。
	\item そうでない場合は、スーパーコンビネータノードに到達するまで、遭遇した関数適用ノードをアンワインドします。
...
\subsection{インスタンス化が行われた後はどうなりますか?}
...
次のスーパーコンビネータを見つけるには、\texttt{Unwind}命令を使用して、再びアンワインドを開始する必要があります。
後置演算子シーケンスに整理とアンワインドを行う操作を追加することで、テンプレートインスタンス化マシンをを Mark 1 Gマシンに変換しました。
\vskip\baselineskip
...
\section{Mark 2: 遅延評価させる} \label{sec:gmachinelazy}
...
アンワインドプロセスがスーパーコンビネータノード \texttt{f} に到達し、スーパーコンビネータが $n$ 個の引数を取るとします。
...
ここで重要な点は、変数へのアクセスが高速になることです(変数が少なくとも1回アクセスされる場合)。
これは、右側の引数を取得するために、関数適用ノードを1回しか見ていないためです。
\vskip\baselineskip
...
\section{Mark 4: プリミティブの追加} \label{sec:gmachineadprim}
...
\subsection{新しい命令遷移} \label{sec_3_6_3}
...
\section{Mark 5: 算術演算のより良い処理に向けて}
...
\begin{tabular} {|l r l r l l|} \hline
	                  & \texttt{Take} $n$ : $i$ & $f$  & $c_1$ : $\ldots$ : $c_n$ : $s$ & $h$                                            & $c$ \\
...
(4.2) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & \texttt{Push} (\texttt{Arg} $k$) : $i$ & $f$ & $s$                  & $h$[$f$ : $\langle (i_1, f_1), \ldots, (i_k, f_k), \ldots, (i_n, f_n) \rangle$] & $c$ \\
...
(4.3) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & \texttt{Push} (\texttt{Label} $l$) : $i$ & $f$ & $s$               & $h$ & $c$[$l$ : $i'$] \\
...
(4.4) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & \texttt{Push} (\texttt{Code} $i'$) : $i$ & $f$ & $s$               & $h$ & $c$ \\
	$\Longrightarrow$ & $i$                                      & $f$ & ($i'$, $f$) : $s$ & $h$ & $c$ \\ \hline
\end{tabular}
\vskip\baselineskip
...
(4.5) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & \texttt{Push} (\texttt{IntConst} $n$) : $i$ & $f$ & $s$                           & $h$ & $c$ \\
...
(4.6) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & [\texttt{Enter} (\texttt{Label} $l$)] & $f$ & $s$ & $h$ & $c$[$l$ : $i$] \\
...
(4.7) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & [\texttt{Enter} (\texttt{Arg} $k$)] & $f$   & $s$ & $h$[$f$ : $\langle (i_1, f_1), \ldots, (i_k, f_k), \ldots, (i_n, f_n) \rangle$] & $c$ \\
...
(4.8) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & [\texttt{Enter} (\texttt{Code} $i$)] & $f$ & $s$ & $h$ & $c$ \\
	$\Longrightarrow$ & $i$                                  & $f$ & $s$ & $h$ & $c$ \\ \hline
\end{tabular}
\vskip\baselineskip
...
(4.9) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & [\texttt{Enter} (\texttt{IntConst} $n$)] & $f$ & $s$ & $h$ & $c$ \\
...
\section{Mark 2: 算術演算の追加}

このセクションでは、マシンに算術演算を追加します。

\subsection{概要: 算術演算の仕組み}
...
\begin{tabular} {|l r l r r l l|} \hline
	                  & \texttt{Op} \texttt{Sub} : $i$ & $f$ & $s$ & $n_1$ : $n_2$ : $v$ & $h$ & $c$ \\
...
(4.12) \\
\begin{tabular} {|l r l r r l l|} \hline
	                  & \texttt{PushV FramePtr} : $i$ & $n$ & $s$ & $v$       & $h$ & $c$ \\
	$\Longrightarrow$ & $i$                           & $n$ & $s$ & $n$ : $v$ & $h$ & $c$ \\ \hline
\end{tabular}

\subsection{実装への単純な算術演算の追加}
...
\section{Mark 3: \texttt{let(rec)}式}
...
\subsection{フレームスロットの再利用 \dag}

現在、スーパーコンビネータ定義の右辺にあるすべての定義は、フレーム内に独自のプライベートスロットを取得します。
...
クロージャがEnterされる直前に、新しい命令 \texttt{PushMarker} $x$ が実行されるようにします。
これにより、ダンプに情報をプッシュすることで更新メカニズムがセットアップされます。
具体的には、\texttt{PushMarker} $x$ はダンプに次の情報をプッシュします
...
(4.15) \\
\begin{tabular} {|l r l r r r l l|} \hline
	                  & \texttt{PushMarker} $x$ : $i$ & $f$ & $s$ & $v$ & $d$               & $h$ & $c$ \\
...
(4.16) \\
\begin{tabular} {|l r l r r r l l|} \hline
	                  & [\texttt{Return}]                                                                                                     & $f$  & []              & $n$ : $v$ & $(f_u, x, s)$ : $d$ & $h$  & $c$ \\
	$\Longrightarrow$ & [\texttt{Return}]                                                                                                     & $f$  & $s$             & $n$ : $v$ & $d$                 & $h'$ & $c$ \\
	                  ...
	                  & [\texttt{Return}]                                                                                                     & $f$  & $(i, f')$ : $s$ & $n$ : $v$ & $d$                 & $h$  & $c$ \\
	$\Longrightarrow$ & $i$                                                                                                                   & $f'$ & $s$             & $n$ : $v$ & $d$                 & $h$  & $c$ \\ \hline
\end{tabular}
\vskip\baselineskip

最初の遷移規則は、フレーム $f_u$ 内の $x$ 番目のクロージャをクロージャ $(\texttt{intCode}, n)$ で更新することを記述します。
これは、Enterされるとすぐに $n$ を値スタックにプッシュしてReturnするクロージャです (セクション 4.3.1 を参照)。
...
\subsection{\texttt{PushMarker}命令のコンパイル}
...
\begin{quote}
	クロージャをコピーするときは非常に注意する必要があります。2つのコピーが存在すると、それらの評価を共有することはできないからです。
\end{quote}
...
2つの \texttt{Push Arg} 命令はそれぞれ同じクロージャのコピーを取得し、その後 \texttt{h} はそれぞれを独立して評価します。
\vskip\baselineskip
...
つまり、共有クロージャをEnterする直前に、評価が完了したときに共有クロージャが更新されるようにする更新メカニズムをセットアップします。
\vskip\baselineskip

アドレッシング モード \texttt{(Code [PushMarker \textit{n}, Enter (Arg \textit{n})])} は、フレームの $n$ 番目のクロージャへの\textbf{間接更新}と呼ばれます。
これは、更新を実行させる間接であるためです。
引数を (スタックにプッシュするのではなく) Enterする場合にも、まったく同じ考慮事項が適用されます。
引数自体ではなく、引数への間接更新をEnterする必要があります。
...
2つの主な問題があり、それらは Argo [Argo 1989] によって初めて識別されました。
...
\texttt{x} を使用するたびに、\texttt{f} はその引数 \texttt{x} への更新クロージャをEnterします。
最初、\texttt{x} はその値で更新されます。
2回目では、2回目の(完全に冗長な)更新が行われ、\texttt{x} がその値で再度上書きされます。
実装でサンプルを実行すると、これが起こるのを確認できるはずです。
\vskip\baselineskip

この場合、もちろん、賢いコンパイラは \texttt{x} が確実に評価されることを認識し、\texttt{x} に間接をEnterする代わりに \texttt{x} をコピーするだけです。
...
\texttt{f} が \texttt{g} を分析して、さまざまな \texttt{x} がどの順序で評価されるかを発見しない限り(そして一般に、この質問に対する答えは1つだけではない可能性があります)、
悲観的に考えて、間接参照の更新を引数として \texttt{g} にプッシュする必要があります。
\vskip\baselineskip

2番目の問題は、\textbf{間接チェーン}の問題です。
次のプログラムを検討してみましょう。
...
ここで、\texttt{f} は引数 \texttt{y} への間接更新演算を \texttt{g} に渡します。
ただし、 \texttt{g} は引数 \texttt{x} への間接更新をEnterします。
したがって、 \texttt{g} は間接への間接をEnterします。
...
これらの問題はまだ解決しません。
代わりに、次のセクションでは、\texttt{let(rec)} バインドされた変数の更新を処理する方法と、これら2つの問題が発生しない理由を示します。
これは、スーパーコンビネータの引数についても、より良い解決策への道を示しています。

\subsection{\texttt{let(rec)}にバインドされた共有変数の更新}
...
つまり、Enterされると、それ自体を更新する更新メカニズムがセットアップされます。
一度この状態が発生すると、\texttt{PushMarker} 命令を含むコードへのポインタが上書きされるため、二度と同じ問題が発生することはありません。
\vskip\baselineskip

一般に、考え方は次のとおりです。

\begin{itemize}
	\item コードを \texttt{PushMarker} 命令で始めることにより、\texttt{let(rec)} 束縛の右辺に自己更新クロージャを使用します。
	\item \texttt{let(rec)} で束縛された変数をスタックにプッシュする場合は、通常の(更新しない)間接アドレッシングモードを使用します。
	      まだ、クロージャのコピーを取得するのではなく、間接アドレッシングを使用する必要があります。
	      これは、クロージャが更新されるまで、コピーすると重複した作業が発生するため、
\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{let(rec)} 式の $\mathcal{R}$ スキームを変更して、すべての右辺のコードの先頭に \texttt{PushMarker} 命令を生成します。
	      これは、新しいコンパイルスキームである $\mathcal{U}$ スキーム(図 4.4 を参照)を作成することで最も簡単に実行できます。
...
\begin{description}
	...
	\item[演習 4.19] 次の表現を考えてみる
		\begin{verbatim}
          let x = 3 in x+x
        \end{verbatim}
		この場合、\texttt{let} 式の右辺はすでに正規形式になっているため、$\mathcal{U}$ スキームで \texttt{PushMarker} 命令を生成する意味はありません。
		むしろ、この場合、$\mathcal{U}$ は単純に \texttt{IntConst} アドレッシングモードを返すことができます。

		$\mathcal{U}$ コンパイルスキームと対応する \texttt{compileU} 関数を変更し、変更が正しく動作することを確認します。
\end{description}

\subsection{間接チェーンの排除}
...
間接チェーンから始めます。
前に述べたように、スーパーコンビネータは引数クロージャをコピーしてはならないと想定する必要があるため、間接チェーンが構築されます。
そのため、それらを複数回使用する場合は、間接を使用することをお勧めします。
...
(この変換は、 \texttt{f} に渡されるクロージャーが複数回Enterされる可能性がある場合にのみ実行する必要があることに注意してください。
ここには、より効率的なコードを生成するためにグローバル共有分析を行う機会があります。)
\vskip\baselineskip

この変換が完了すると、引数クロージャを自由にコピーできますが、\texttt{let(rec)} 束縛されたクロージャには(更新されない)間接参照を使用する必要があります。
...
したがって、遅延更新の問題により、TIMはよりGマシンらしくなる必要がありました。
\vskip\baselineskip

この手法を、引数式ごとに \texttt{let} 式を導入するプログラム変換として紹介しましたが、これを行うには、新しい任意の変数名を考え出す必要があるため、少々面倒です。
新しいコンパイルスキームをより直接的に記述する方が簡単です。
...
\begin{tabular} {|l|} \hline
	...
	\\
	\begin{tabular}{l c l}
		\multicolumn{3}{l}{$\mathcal{R} \llbracket e ~ a \rrbracket ~ \rho ~ d$}                                                                                  \\
		 & $=$ & ($d_1$, \texttt{Push} ($\mathcal{A} \llbracket a \rrbracket ~ \rho$) \texttt{:} $is$)                                                            \\
		 &     & ここで、$a$ はスーパーコンビネータ、局所変数、                                                                                                   \\
		 &     & または整数であり、$d_1, ~ is$は以下の通りです。                                                                                                  \\
		 &     & $(d_1, is) = \mathcal{R} \llbracket e \rrbracket ~ \rho ~ d$                                                                                     \\
		\multicolumn{3}{l}{$\mathcal{R} \llbracket e_{fun} ~ e_{arg} \rrbracket ~ \rho ~ d$}                                                                      \\
		 & $=$ & ($d_2$, \texttt{Move} (d + 1) $am_{arg}$ \texttt{:} \texttt{Push} (\texttt{Code} [\texttt{Enter} (\texttt{Arg} (d + 1))]) \texttt{:} $is_{fun}$) \\
...
(4.17) \\
\begin{tabular} {|l r l r r r l l|} \hline
	                  & \texttt{UpdateMarkers} $n$ : $i$                                                                                       & $f$ & $c_1$ : $\ldots$ : $c_m$ : $s$ & $v$ & $d$                 & $h$  & $c$ \\
	$\Longrightarrow$ & $i$                                                                                                                    & $f$ & $c_1$ : $\ldots$ : $c_m$ : $s$ & $v$ & $d$                 & $h$  & $c$ \\
	                  & \multicolumn {7} {l|} {ここで$m \ge n$}                                                                                                                                                                \\ \hline
	                  & \texttt{UpdateMarkers} $n$ : $i$                                                                                       & $f$ & $c_1$ : $\ldots$ : $c_m$ : []  & $v$ & $(f_u, x, s)$ : $d$ & $h$  & $c$ \\
...
\begin{itemize}
	...
	\item $i'$ は以下の符号列でなければなりません
	      ...
	      \vskip\baselineskip
	      ここで、\texttt{Push} 命令は部分適用の引数をスタックに配置し、\texttt{UpdateMarkers} 命令は実行する必要があるさらなる更新をチェックします。
...
新しい機能は、式 \texttt{y + sum ys} がリストセルのコンポーネント \texttt{y} と \texttt{ys} を使用することです。
...
2つの \texttt{Move} 命令は、\texttt{y} と \texttt{ys} をリストセルから現在のフレーム (\texttt{xs} を含むフレーム) にコピーします。
\vskip\baselineskip
...
(4.18) \\
\begin{tabular} {|l r l l r r r l l|} \hline
	                  & $[\texttt{Switch} ~ [\ldots ~ t ~ \texttt{->} ~ i ~ \ldots]]$ & $f$ & $f_d$ & $s$ & $t$ : $v$ & $d$ & $h$ & $c$ \\
...
(4.19) \\
\begin{tabular} {|l r l l r r r l l|} \hline
	                  & $[\texttt{ReturnConstr} ~ t]$ & $f$  & $f_d$ & $(i, ~ f')$ : $s$ & $v$       & $d$ & $h$ & $c$ \\
...
(4.20) \\
\begin{tabular} {|l r l l r r r l l|} \hline
	                  & $[\texttt{ReturnConstr} ~ t]$                                                                                                                 & $f$ & $f_d$ & []  & $v$ & $(f_u, ~ x, ~ s)$ : $d$ & $h$  & $c$ \\
	$\Longrightarrow$ & $[\texttt{ReturnConstr} ~ t]$                                                                                                                 & $f$ & $f_d$ & $s$ & $v$ & $d$                     & $h'$ & $c$ \\
...
\begin{tabular} {|l|} \hline
	...
	\\
	\begin{tabular}{l c l}
		\multicolumn{3}{l}{$\mathcal{E} \llbracket \texttt{<} t \texttt{>} ~ x_1 ~ \ldots ~ x_n ~ \texttt{->} ~ body \rrbracket ~ \rho ~ d$} \\
		 & $=$ & ($d'$, $t$ \texttt{->} ($is_{moves}$ \texttt{++} $is_{body}$))                                                              \\
...
この場合、数値は値スタックで返されるため、\texttt{Print} は値スタックから数値を取り出し、それを出力に追加します。
...
\section{Mark 6: Constant Applicative Forms (定作用形)とコードストア \dag}
...
CAFが呼び出されるとき、redexのルートはスーパーコンビネータノード自体であるため、ノードはリダクションの結果(つまり、スーパーコンビネータ定義式の右辺のインスタンス)で更新されます。
...
\texttt{Push Label} の新しい遷移規則は、これらのアイデアを形式化します。
\vskip\baselineskip
...
\texttt{Enter Label} の遷移規則は、\texttt{Push}/\texttt{Enter} 関係から直接に従います。
...
CAFの適切な更新を Mark 4 または Mark 5 TIM に追加するには、次の手順を実行する必要があります。

\begin{itemize}
	\item マシン状態のコード ストア コンポーネントには、グローバル フレームのアドレスと、スーパーコンビネータ名とフレーム オフセット間の関連付けが含まれるようになりました。
	      \begin{verbatim}
          type CodeStore = (Addr, ASSOC Name Int)
        \end{verbatim}
	      この変更を考慮するには、\texttt{showSCDefns} 関数を変更する必要があります。
	\item 関数 \texttt{amToClosure} は、上で説明したように、\texttt{Label} アドレッシング モードに対して異なるアクションを実行する必要があります。
	\item \texttt{compile} 関数で計算された初期環境 \texttt{initial\_env} は、スーパーコンビネータごとに間接アドレス指定モードを生成するように変更する必要があります。
	\item 最後の変更にはほとんどの作業が含まれます。
	      テンプレートインスタンス化マシンと G マシンの \texttt{compile} 関数で行ったのと同じように、初期ヒープを構築するには \texttt{compile} 関数を変更する必要があります。
\end{itemize}

これらの項目の最後の部分については、もう少し議論する必要があります。
\texttt{compile} では、空のヒープから開始するのではなく、補助関数 \texttt{allocateInitialHeap} を使用して初期ヒープを構築する必要があります。
\texttt{allocateInitialHeap} には、\texttt{compile} 関数から \texttt{compiled\_code} が渡されます。
これは、\texttt{compiled\_code} の各要素のクロージャを含む単一の大きなフレームを割り当て、初期ヒープと \texttt{codeStore} を返します。
...
\texttt{allocateInitialHeap} は次のように機能します。
まず、各要素を1から始まるフレームオフセットと組み合わせることで、\texttt{compiled\_code} リストにインデックスが付けられます。
このリストは個別に処理されて、スーパーコンビネータ名からアドレスへのマッピングである \texttt{offsets}、
およびグローバルフレームに配置されるクロージャのリストである \texttt{closures} が生成されます。
最後に、グローバルフレームが割り当てられ、結果のヒープがコードストアとともに返されます。
\vskip\baselineskip

\texttt{global\_frame\_addr} はクロージャの構築に使用されることに注意してください。
各スーパーコンビネータクロージャのフレームポインタはグローバルフレームポインタそのものであるため、
\texttt{PushMarker} 命令はグローバルフレームを参照する更新フレームをプッシュします。

\begin{description}
	\item[演習 4.27] \texttt{showSCDefns}、\texttt{compileA}、\texttt{amToClosure}、および \texttt{compile} に必要な変更を加えます。
		CAFの更新が実際に行われるかどうかをテストします。
	\item[演習 4.28] \texttt{allocateInitialHeap} 内に追加された \texttt{PushMarker} 命令はCAFにのみ必要であり、1つ以上の引数を持つスーパーコンビネータにとっては時間の無駄です。
		CAFに対してのみ \texttt{PushMarker} 命令を植えるように \texttt{allocateInitialHeap} を変更します。
		(ヒント: 非CAFは、そのコードが \texttt{Take} $n$ 命令 $(n > 0)$ で始まるという事実によって識別できます。)
		改善を測定します。
	\item[演習 4.29] 間接アドレス指定モードはCAFにのみ必要であり、非CAFスーパーコンビネータには必要ありません。
		この事実を利用するには、\texttt{initial\_env} の構造を変更します。
\end{description}

\subsection{コードストアをより忠実にモデリングする}

これまでの \texttt{Label} の扱いには少し奇妙な点があります。
それは次のとおりです。
スーパーコンビネータの名前は、コンパイル時に (\texttt{Label} アドレッシングモードにマッピングするため) 環境内で検索され、
次に実行時に (グローバルフレームのオフセットにマッピングするために) 再度検索されます
\footnote{前のセクションで提案した変更を CAF に対して実装したことを前提としていますが、このセクションは CAF 以前のバージョンのマシンにも適用されます。}。
これは現実的ではありません。
実際のコンパイラでは、名前が検索されます。
ただし、実行前にハードマシン アドレスにリンクされるため、実行時の検索は行われません。
\vskip\baselineskip

これをモデル化するには、\texttt{Label} コンストラクタを1つではなく2つの引数を取るように変更します。
次のようになります。
...
\texttt{name} フィールドには以前と同様にスーパーコンビネータの名前が記録されますが、\texttt{num} はグローバルフレームで使用するオフセットを示します。
テンプレートインスタンス化マシンの \texttt{NSupercomb} コンストラクタと同様に、\texttt{name} フィールドはドキュメントとデバッグの目的でのみ存在します。
\texttt{Push Label} の遷移規則が改訂されていることからわかるように、コードストアコンポーネントは単にグローバルフレームのアドレスになりました。
\vskip\baselineskip

(4.22) \\
\begin{tabular} {|l r l r l l|} \hline
	                  & \texttt{Push} (\texttt{Label} $l$ $k$) : $i$ & $f$ & $s$                  & $h$[$g$ : $\langle (i_1, f_1), \ldots, (i_n, f_n) \rangle$] & $g$ \\
	$\Longrightarrow$ & $i$                                          & $f$ & ($i_k$, $f_k$) : $s$ & $h$                                                         & $g$ \\ \hline
\end{tabular}
\vskip\baselineskip

\texttt{Enter} の遷移規則は、\texttt{Push}/\texttt{Enter} 関係に従います。

\begin{description}
	\item[演習 4.30] このアイデアを実装します。
		これをするには：
		\begin{itemize}
			\item 説明に従って \texttt{timAMode} 型を変更します。
			\item \texttt{codeStore} 型をフレームポインタのみで構成されるように変更します。
			\item マシンの正しい初期状態を生成するように \texttt{compile} 関数を変更します。
			      特に、適切な \texttt{Label} アドレッシングモードを使用して、\texttt{initial\_env} を生成する必要があります。
			\item これらの変化を考慮して \texttt{show} 関数を調整します。
		\end{itemize}
\end{description}
\newpage
...
\begin{tabular} {|l|} \hline
	...
	コードは、フレームの最初の $d$ スロットが占有されていると想定し、                                                     \\
	スロット $(d + 1, ~ \ldots, ~ d')$ を使用します。                                                                     \\
	\\
	\begin{tabular}{l c l}
		\multicolumn{3}{l}{$\mathcal{E} \llbracket \texttt{<} t \texttt{>} ~ x_1 ~ \ldots ~ x_n ~ \texttt{->} ~ body \rrbracket ~ \rho ~ d$} \\
...
\begin{tabular} {|l|} \hline
	...
	\end{tabular}
	\\ \hline
	$\mathcal{I} \llbracket d \rrbracket$ はオフセット $d$ のフレームの間接アドレッシングモードです。      \\
	\\
	$\mathcal{I} \llbracket d \rrbracket$ = \texttt{Code} [\texttt{Enter} (\texttt{Arg} $d$)]
	\\ \hline
	$\mathcal{B} \llbracket e \rrbracket ~ \rho ~ d ~ cont$ はペア $(d', ~ is)$ です。                     \\
	...
	\\
	\begin{tabular}{r c l}
		$\mathcal{B} \llbracket e_1 \texttt{+} ~ e_2 \rrbracket ~ \rho ~ d ~ cont$ & $=$                                                                   & $\mathcal{B} \llbracket e_2 \rrbracket ~ \rho ~ d_1 ~ is_1$                                      \\
		                                                                           & $=$                                                                   & $(d_1, ~ is_1) = \mathcal{B} \llbracket e_1 \rrbracket ~ \rho ~ d ~ (\texttt{Op Add :} ~ cont))$ \\
		                                                                           & \multicolumn {2} {l} {$\ldots$および他の算術プリミティブの同様の規則}                                                                                                    \\
		\\
		$\mathcal{B} \llbracket n \rrbracket ~ \rho ~ d ~ cont$                    & $=$                                                                   & ($d$, \texttt{PushV} (\texttt{IntVConst} $n$) \texttt{:} $cont$)                                 \\
		                                                                           &                                                                       & ここで、$n$は数値です。                                                                          \\
		\\
		$\mathcal{B} \llbracket e \rrbracket ~ \rho ~ d ~ cont$                    & $=$                                                                   & ($d'$, \texttt{Push} (\texttt{Code} $cont$) \texttt{:} $is$)                                     \\
...
\texttt{dsum} 関数は、問題をほぼ等しい2つの部分に分割することによって動作します。
次に、2つの結果を組み合わせて答えを生成します。
これは古典的な\textbf{分割統治アルゴリズム}です。
\vskip\baselineskip

関数 \texttt{dsum} と \texttt{psum} のどちらにも、言語内の並列プリミティブについての言及が含まれていないことに注意してください。
では、なぜ \texttt{psum} は並列アルゴリズムなのでしょうか?
比較のために、逐次アルゴリズム \texttt{ssum} を作成できます。

\begin{verbatim}
  ssum n = if (n == 1) 1 (n + ssum (n - 1))
\end{verbatim}

この関数は、\textbf{データの依存関係}が本質的に逐次的であるため、逐次アルゴリズムです。
\texttt{ssum} の例では、これが意味することは、\texttt{ssum n} の加算は \texttt{ssum (n - 1)} が評価された後にのみ実行できるということです。
これは、\texttt{ssum (n - 2)} などを評価した後にのみ実行できます。
この違いを次のように要約できます。
...
他のプログラミング言語と同様に、並列アルゴリズムが不可欠です。
ただし、並列命令型プログラミングとは対照的であることに注意してください。

\begin{itemize}
	\item 並列処理、同期、通信を表現するために新しい言語構造を構築する必要はありません。
	      同時実行性は暗黙的であり、新しいタスクが動的に作成され、マシンに空き容量があるときはいつでもマシンによって実行されます。
	\item 同時タスク間で共有されるデータを保護するための特別な対策は講じられていません。
	      たとえば、\texttt{mid} は \texttt{dsum} の 2 つの同時タスク間で安全に共有されます。
	\item 逐次実行される関数型プログラムに使用するすべての手法が引き続き機能するため、並列プログラムについて推論するために新しい証明手法は必要ありません。
	      また、デッドロックは次のような自己依存性の結果としてのみ発生する可能性があることにも注意してください。
	      \begin{verbatim}
          letrec a = a + 1 in a
        \end{verbatim}
	      自身の値に依存する式は無意味であり、その非停止性は逐次実装で観察される動作と同じです。
	\item プログラムの結果は決定的です。
	      スケジューリングアルゴリズムでは、同じプログラムの2つの実行間で回答が異なることはあり得ません。
\end{itemize}

要約すると、これらの機能により、多数の低レベルの問題を解決する必要なく、並列アルゴリズムを簡単に表現できることがわかります。
おそらくこれは次のように特徴づけることができます。

\begin{quote}
	並列命令型プログラムは、並列関数型プログラムでは言及すらしないリソース割り当ての決定を詳細に指定します。
\end{quote}

これは、マシンがリソース割り当ての決定を自動的に行うことができる必要があることを意味します。
これらの決定が最適でない場合には、実行効率を損なうことになり、その代償を支払うことになります。
\vskip\baselineskip

\textbf{アノテーション}
\vskip\baselineskip

関数型言語によって提供される高度な抽象化により、コンパイル時および実行時のリソースアロケータに大きな要求が課されます。
すべてのリソース割り当ての決定をシステムに任せるのではなく、新しい並列スレッドを開始するアノテーション \texttt{par} を導入します。
アノテーションは、プログラムテキストの意味を保持する装飾です。 \texttt{par} の場合、次の構文と意味になります。

\[
	\texttt{par} ~ E_1 ~ E_2 =  E_1 ~ E_2
\]

つまり、\texttt{par} は関数適用の同義語です。
異なる点は、式 $E_2$ が同時タスクによって評価されることを意図していることです。
例として、このアノテーションを使用して \texttt{dsum} 関数を書き直します。
...
\texttt{par} によって2番目の引数 \texttt{+} が並列評価されることがわかります。
\texttt{par} アノテーションはプログラマによって挿入できますが、原理的には賢いコンパイラによって挿入できます。
ただし、そのような賢さについては本書の範囲を超えているため、\texttt{par} はすでに挿入されているものと仮定します。

\subsection{並列グラフ簡約}

この本では、グラフ簡約が逐次マシンにとって有用な実装手法であることを説明しました。 並列マシンの実装にも適していることは驚くことではありません。
実際、これには多くの利点があります。

\begin{itemize}
	\item プログラムカウンタには連続的な概念はありません。グラフの簡約は分散化され、配布されます。
	\item 簡約はグラフ内の多くの場所で同時に実行される可能性があり、スケジュールされた相対的な順序が結果に影響することはありません。
	\item すべての通信と同期はグラフを介して行われます。
\end{itemize}
\vskip\baselineskip
...
\textbf{タスク}は、特定のサブグラフを WHNF に還元することを目的とした逐次計算です。
いつでも、実行できるタスクが多数存在する可能性があります。 このスパークされたタスクのコレクションは、\textbf{スパークプール}と呼ばれます。
作業を探しているプロセッサは、スパークプールから新しいタスクをフェッチし、それを実行します。
タスクは\textbf{仮想プロセッサ}と考えることができます。
\vskip\baselineskip

最初は、プログラム全体を評価するタスクが1つだけあります。
実行中に、メインタスクが後で必要とする式を評価するための新しいタスクが作成されることが期待されます。
これらはスパークプールに配置され、仕事がなくなった場合に別のプロセッサがタスクを引き継ぐことができます。
タスクをスパークプールに配置する行為を、子タスクの\textbf{スパーク}と呼びます。
\vskip\baselineskip

親タスクと子タスク間の相互作用を考慮すると便利です。
タスク管理の評価して終了するモデルでは、各グラフノードにロックビットがあります。
ビットがオンの場合、ノードを評価するタスクが実行されています。 それ以外の場合、ビットはオフです。
親タスクが、タスクがスパークされたサブグラフの値を必要とする場合、考慮すべきケースが 3 つあります。

\begin{itemize}
	\item 子タスクはまだ開始されていません。
	\item 子タスクは開始されましたが、まだ停止していません。
	\item 子タスクが完了しました。
\end{itemize}

最初のケースでは、親タスクは、あたかも子タスクが存在しないかのようにグラフを評価できます。
もちろん、これによりロック ビットが設定されるため、子タスクを実行しようとするとすぐに破棄されます。
興味深いのは2番目のケースで、親と子の両方が実行されています。
このような状況の場合、親タスクは子タスクが完了するまで待ってから続行する必要があります。
子タスクが親タスクを\textbf{ブロック}していると言います。
3番目のケースでは、ノードは現在 WHNF にあり、ロックが解除されているため、親タスクがグラフの値をフェッチするのにそれほど時間はかかりません。
\vskip\baselineskip

評価して終了するモデルの利点は、親と子が実際に衝突した場合にのみブロッキングが発生することです。
それ以外の場合はすべて、親の実行は妨げられることなく続行されます。
ブロックメカニズムがタスク間通信および同期の\textbf{唯一の}形式であることに注意してください。
グラフの一部が WHNF に対して評価されると、任意の数のタスクが競合することなく同時にそれを検査できます。
\vskip\baselineskip
...
後で説明するように、恒等関数の2つの簡約は並行して行われます。
\vskip\baselineskip

優れたコンパイラは、\texttt{main} スーパーコンビネータに対して次のコードを生成します。\footnote{コンパイラが十分に洗練されていない場合、このコードを生成しない可能性があります。}
...
最初は、実行中のタスクは1つだけで、部分式 \texttt{(I 3)} を構築しています。
コードシーケンス \texttt{e1} を実行した後、マシンは図 5.1 の図 (a) に示す状態になります。
\vskip\baselineskip

\texttt{e1} を実行した後、マシンは \texttt{Par} 命令に遭遇します。
これにより、スタックの先頭から指定されたノードを評価するための新しいタスクが作成されます。
新しいタスク \texttt{Task 2} と呼びます。元のタスクは \texttt{Task 1} と呼ばれます。
この状況は、図 5.1 の図 (b) に示されています。
\vskip\baselineskip

\includegraphics*[scale=0.5]{Fig_5-1.png}

図 (c) (図 5.2) では、\texttt{Task 1} が評価を続行しており、\texttt{Pushglobal I} 命令を実行していることがわかります。
新しく作成されたタスク (\texttt{Task 2}) は、コードシーケンス:\texttt{[Unwind]} で始まります。したがって、評価するように割り当てられたグラフのアンワインドを開始します。
図 (d) は、\texttt{Task 1} が \texttt{main} の本体のインスタンス化を完了し、\texttt{Task 2} がアンワインドを完了していることを示しています。
\vskip\baselineskip

\includegraphics*[scale=0.5]{Fig_5-2.png}

本体がインスタンス化されると、\texttt{Task 1} は \texttt{main} である redex ノードを上書きします。
\texttt{Task 2} は \texttt{Push 0} 命令を実行します。これは \texttt{I} スーパーコンビネータのコードの最初の命令です。
これは図 (e) に示されています (図5.3)。
図 (f) では、\texttt{Task 1} がスパインをアンワインドし始め、\texttt{Task 2} が更新を実行していることがわかります。
\vskip\baselineskip

\includegraphics*[scale=0.5]{Fig_5-3.png}

図 5.4 では、\texttt{Task 2} が完了するまで実行されています。
\texttt{Task 1} の残りの実行は、シーケンシャル G マシンと同じなので省略します。
\vskip\baselineskip

\includegraphics*[scale=0.5]{Fig_5-4.png}

これで、並列 G マシンの同時タスク実行の概要は終了です。
最小限の並列 G マシンを提供します。
\newpage

\section{Mark 1: 最小限の並列Gマシン}

最初に紹介するマシンは、第3章のGマシンのいずれか (Mark 1 を除く) をベースにすることができます。
更新を確実に行うには、この制限が必要です。
この基本マシンに、並列処理のための機構を追加する必要があります。
次の基本的な仮定を立てます。

\begin{enumerate}
	\item すべてのプロセッサがアクセスできる共有グローバル グラフがあります。
	\item プロセッサの数は無限です。つまり、タスクを実行できるプロセッサが常に存在します。
	\item グラフ ノードはロックされません。つまり、異なるタスクが同じ式を再評価する可能性があります。
\end{enumerate}

\subsection{データ型の定義}

この章では、状態アクセス関数を多用します。
特に興味深いものではありませんが、完全性を保つためにこのセクションに含めています。
\vskip\baselineskip

並列マシンでは、状態 \texttt{pgmState} は、グローバルコンポーネント \texttt{pgmGlobalState} とローカルコンポーネント \texttt{pgmLocalState} の 2 つのコンポーネントに分割されます。
\texttt{pgmLocalState} には、プログラムを実行するプロセッサが含まれます。
\texttt{pgmGlobalState} には、プロセッサによって操作されるグローバル データ構造 (最も頻繁に使用されるのはヒープ) が含まれます。
...
\textbf{グローバル状態コンポーネント}
\vskip\baselineskip

並列実装のベースとして使用できる可能性のあるすべてのマシンに対応するために、グローバル状態は5つのコンポーネントで構成されます。
プログラムの評価に対する回答として出力される出力である \texttt{gmOutput}、
ヒープである \texttt{gmHeap}、
各スーパーコンビネータのヒープ内の一意のノードを指すために使用される \texttt{gmGlobals}、
タスクプールであり、タスクの実行を開始する前にタスクを保持するために使用される \texttt{gmSparks}、
およびグローバルクロックである \texttt{gmStats} です。
...
\begin{itemize}
	...
	\item スパークプールは、\texttt{gmSparks} コンポーネントによって表されます。
        ...
        \end{verbatim}
	\item 最後に、並列Gマシンでは、グローバルコンポーネントに蓄積された統計情報を保持します。
	      これは数値のリストとして表され、マシン内の各タスクが完了するまでに実行された時間の詳細を示します。
	      \begin{verbatim}
          type GmStats = [Int]
        \end{verbatim}
	      このコンポーネントへのアクセスは、\texttt{pgmGetStats} を使用して行われます。
	      \begin{verbatim}
...
並列Gマシンのローカルコンポーネントはプロセッサのリストで構成されます。
プロセッサはタスクとして表されます。

ここでも、並列マシンを任意のGマシンをベースとして実行できるようにするために、各プロセッサの状態を5要素のタプルにします。
...
\begin{itemize}
	\item コードシーケンスは単なる命令のリストです。
	      \begin{verbatim}
          type GmCode = [Instruction]
        \end{verbatim}
	\item シーケンシャルGマシンと同様に、スタックはヒープ内のアドレスのリストです。
	      \begin{verbatim}
          type GmStack = [Addr]
        \end{verbatim}
	\item 並列実装のベースとして Mark 4 Gマシン (またはそれ以降のマーク) を使用している場合は、ダンプが必要です。
	      これは、それぞれが \texttt{gmDumpItem} 型のダンプ項目のスタックとして使用されます。
	      \begin{verbatim}
          type GmDump = [GmDumpItem]
          type GmDumpItem = (GmCode, GmStack)
        \end{verbatim}
	\item 実装のベースとして Mark 7 Gマシンを使用した場合は、プロセッサごとにVスタックが必要になります。
	      \begin{verbatim}
          type GmVStack = [Int]
        \end{verbatim}
	\item また、各プロセッサにクロックも提供します。
	      これにより、タスクが実行した命令の数を記録します。
	      \begin{verbatim}
          type GmClock = Int
        \end{verbatim}
\end{itemize}
...
並列マシンの状態アクセス関数はすでにいくつか定義されていますが、さらにいくつか定義しておくと便利です。
各プロセッサは1つの状態遷移を行い、その間はシーケンシャルマシンのように動作します。
状態 \texttt{gmState} を、現在のマシン状態のグローバルコンポーネントと単一のプロセッサ状態からなるペアにすると、
シーケンシャルGマシンの状態コンポーネントのスーパーセットが得られます。

\begin{verbatim}
  type GmState = (PgmGlobalState, PgmLocalState)
\end{verbatim}

したがって、シーケンシャル マシンで使用した状態アクセス関数を、新しいタイプの状態で動作するように単純に再定義できます。
グローバル \texttt{put} 関数の型シグネチャは次のとおりです。
...
プロセッサ上で単一のステップを実行するには、\texttt{step} 関数を使用します。
...
\texttt{doAdmin} 関数は、実行を終了したプロセッサを削除します。
コードコンポーネントが空になると、プロセッサは終了しています。
この場合、プロセッサがタスクを完了するのに要した命令の数で、状態の統計コンポーネントを更新する必要があります。
...
追加する必要がある唯一の新しい命令は \texttt{Par} です。
その効果は、マシンがノードを WHNF に評価するタスクを作成できるように、スタックの最上部のノードをマークすることです。
これを行うには、命令は、ノードのアドレスをスパークプールに追加して、状態のグローバルコンポーネントを変更する必要があります。
\vskip\baselineskip

(5.1) \\
\begin{tabular} {|l r l l l l r r r l|} \hline
	                  & $\langle$ & $h$ & $m$ & $t$                & $\rangle$ & $\langle$ & \texttt{Par :} $i$ & $a$ \texttt{:} $s$ & $\rangle$ \\
	$\Longrightarrow$ & $\langle$ & $h$ & $m$ & $a$ \texttt{:} $t$ & $\rangle$ & $\langle$ & $i$                & $s$                & $\rangle$ \\ \hline
\end{tabular}
\vskip\baselineskip

最初のタプル($h$、$m$、$t$ から構成)はグローバル状態コンポーネントで、$h$、$m$、$t$ はそれぞれヒープ、グローバルアドレスマップ、スパークプールです。
2番目のタプル(命令ストリームとスタックから構成)は特定のタスクのローカル状態です。
ベースとして使用したGマシンのバージョンによっては、ローカル状態に他のコンポーネントを追加する必要がある場合があります。
\vskip\baselineskip

\texttt{Par} の効果は、アドレス $a$ をスパークプールに追加することです。
次のように実装されます。
...
\begin{description}
	\item[演習 5.2] \texttt{Par} が正しく処理されるように、\texttt{showInstruction}, \texttt{dispatch}, \texttt{instruction} を変更します。
\end{description}

\subsection{プログラムのコンパイル}

\texttt{par} 関数のコンパイル済みプリミティブを提供することで、シーケンシャルマシン (Mark 7 を除く) のいずれかに基づく並列マシン用のシンプルなコンパイラを構築できます。
Mark 7 ベースのマシンでは、より広範な変更が必要です。
\vskip\baselineskip

他に必要な変更は \texttt{compile} 関数にあります。
ここでは、さまざまなコンポーネントがさまざまな場所に存在するようになり、もちろんプロセッサの数も増えました。
新しい定義は次のとおりです。
...
これにより、シーケンシャルGマシンで使用されるように、グローバルコンポーネントがヒープとグローバルマップを保持するよう設定されます。
また、実行を開始するために、ローカルコンポーネントにタスクを配置します。
...
Mark 2 または Mark 3 シーケンシャルGマシンをベースとして使用する場合は、\texttt{initialCode} を次のように定義する必要があります。
...
また、データ構造を処理するために、Mark 6 および Mark 7 マシンの \texttt{initialCode} は次のようになります。

\begin{verbatim}
  initialCode = [Eval, Print]
\end{verbatim}

ここで、マシンの基本機能に \texttt{par} を追加する方法を検討します。
まず、シーケンシャルGマシン Mark 2 から 6 に基づくマシンを検討します。
\vskip\baselineskip

\textbf{Mark 2-6 Gマシンをベースとして}
\vskip\baselineskip

\texttt{compiledPrimitives} の定義には、次の内容を含める必要があります。

\begin{verbatim}
  ("par", 2, [Push 1, Push 1, Mkap, Push 2, Par, Update 2, Pop 2, Unwind])
\end{verbatim}

このかなり難解なコードは、関数 \texttt{par} が 2 つの引数 $E_1$ と $E_2$ に適用されたときに、次のタスクを実行します。

\begin{enumerate}
	\item まず、$E_1$ を $E_2$ に適用します。これがシーケンスの役割です。
	      \begin{verbatim}
        	[Push 1, Push 1, Mkap]
        \end{verbatim}
	\item 次に、\texttt{Push 2} は $E_2$ へのポインタのコピーを作成します。
	      次に、\texttt{Par} 命令がこのアドレスをスパークプールに追加します。
	\item 最後に、インスタンス化後に通常の更新と整理を実行します。
\end{enumerate}
\vskip\baselineskip

\textbf{Mark 7 Gマシンをベースに}
\vskip\baselineskip

上で述べたように、これは少しトリッキーな操作です。
\texttt{par} を含む特殊なケースを認識するために、コンパイラ関数 \texttt{compileR} と \texttt{compileE} を変更する必要があります。
まず、\texttt{compileR} に次のケースを追加する必要があります。
...
これは $\mathcal{C}$ スキームを使用して \texttt{e2} をコンパイルし、その後スパークします。
式 \texttt{e1} は、アプリケーションノードを作成する前に $\mathcal{C}$ スキームを使用してコンパイルされます。
最後に、スタックの更新と整理を実行します。
\vskip\baselineskip

次に、\texttt{compileE} を修正して、次のケースを作成します。
...
これは、作成されるアプリケーションノードを WHNF に強制するために \texttt{Eval} を使用するという点でのみ、\texttt{compileR} の場合と異なります。
\vskip\baselineskip

コンパイラにこれら2つの変更を加えたら、プリミティブに次のコードを追加するだけで十分です。
...
\begin{description}
	\item[演習 5.3] \texttt{par} 関数が定義されるようにコンパイラを変更します。

	\item[演習 5.4] \texttt{compileR} および \texttt{compileE} の特殊なケースで、最初の引数を構築する前に2番目の引数のグラフのスパークを実行するのはなぜですか?
\end{description}
...
\section{Mark 2: 評価して終了するモデル}

Mark 1 マシンの問題は、ヒープ内の同じノードを簡約するために多くのタスクを作成し、その結果マシンによって実行される作業が重複するリスクがあることです。
これを防ぐ方法は、アンワインド中にノードを\textbf{ロック}することです。
これにより、同じノードに遭遇する他のすべてのタスクが\textbf{ブロック}されます。
また、ノードが再び解放されたら、ノードの\textbf{ロックを解除}することを忘れないようにしてください。
これにより、ブロックされたタスクが再開されます。
\vskip\baselineskip

ロックされていないノードをロック状態にする唯一の命令は \texttt{Unwind} です。
これは、スパイン上の各ノードに発生します。
\texttt{Eval} ではなくこの命令を選択する理由は、スパインをロック解除する途中でロックされたノードに遭遇する可能性があるためです。
\texttt{Eval} を使用すると、このケースをキャッチできません。
同様に、タスクをブロックする唯一の命令は \texttt{Unwind} です。
結局のところ、ノードの値を検査する必要があるのは \texttt{Unwind} だけです。
\vskip\baselineskip

逆に、ロックされたノードのロックを解除する唯一の命令は \texttt{Update} です。
以前にロックされたノードは、WHNF であることがわかっている場合にロック解除されます。
しかし、Redex を更新するときには、Redex の下のスパイン内のすべてのノードが WHNF であることがわかっているため、Redex のルートの下のすべてのノードのロックを解除する必要があります。

\subsection{ノードデータ構造}

私たちが取り入れようとしている改善には、マシンのデータ構造にほとんど変更を加える必要がありません。
まず、ノードデータ型に2種類の新しいノードを追加する必要があります。
これらは、ロックされたアプリケーションノードである \texttt{NLAp} と、ロックされたスーパーコンビネータノードである \texttt{NLGlobal} です。
新しい命令遷移のセクションで、これらがどのように使用されるかを見ていきます。
...
\begin{description}
	\item[演習 5.8] 新しくロックされたノードを処理するために、\texttt{showNode} 関数を書き直します。
\end{description}

\subsection{命令セット}

必要な変更は、適切な場所でノードをロックおよびロック解除することだけです。
アンワインド時に、関数適用ノードとスーパーコンビネータを引数なしでロックする必要があります。
ノードが更新されると、そのノードの下のスパインにあるすべてのノードのロックを解除する必要があります。
ヒープノードのロックとロック解除を実行するには、2つの関数を使用します。
\texttt{lock} 関数は、ロックされていないが更新可能な可能性のあるノードをロックされたノードに変換します。
ロックする必要があるノードは、関数適用ノードと引数のないグローバルノードです。
...
ロックされた関数適用ノードを \texttt{unlock} する場合、そのノードが指すスパインもロック解除されていることを確認する必要があります。
したがって、\texttt{unlock} は再帰的です。
...
\texttt{Unwind} と \texttt{Update} の新しいステップ遷移は、\texttt{lock} と \texttt{unlock} の観点から定義する必要があります。
これらの遷移は現在定義されています。
遷移規則 5.2 では、関数適用ノード(\texttt{*NAp} で表される)をロックすること以外は、\texttt{Unwind} は Mark 1 マシンと同じ遷移規則を持つことがわかります。
\vskip\baselineskip

(5.2) \\
\begin{tabular} {|l r l l l l r r r l|} \hline
	                  & $\langle$ & $h[a : \texttt{NAp} ~ a_1 ~ a_2]$  & $m$ & $t$ & $\rangle$ & $\langle$ & [\texttt{Unwind}] & $a$ \texttt{:} $s$                  & $\rangle$ \\
	$\Longrightarrow$ & $\langle$ & $h[a : \texttt{*NAp} ~ a_1 ~ a_2]$ & $m$ & $t$ & $\rangle$ & $\langle$ & [\texttt{Unwind}] & $a_1$ \texttt{:} $a$ \texttt{:} $s$ & $\rangle$ \\ \hline
\end{tabular}
\vskip\baselineskip

同じことは、スタックの一番上にアリティ0のスーパーコンビネータがある場合の \texttt{Unwind} の遷移規則にも当てはまります。
この場合、\texttt{*NGlobal} はロックされたグローバルノードです。
\vskip\baselineskip

(5.3) \\
\begin{tabular} {|l r l l l l r r r l|} \hline
	                  & $\langle$ & $h[a : \texttt{NGlobal} ~ 0 ~ c]$  & $m$ & $t$ & $\rangle$ & $\langle$ & [\texttt{Unwind}] & $a$ \texttt{:} $s$ & $\rangle$ \\
	$\Longrightarrow$ & $\langle$ & $h[a : \texttt{*NGlobal} ~ 0 ~ c]$ & $m$ & $t$ & $\rangle$ & $\langle$ & $c$               & $a$ \texttt{:} $s$ & $\rangle$ \\ \hline
\end{tabular}
\vskip\baselineskip

\texttt{Update} の新しい遷移規則では、アドレスが $a$ である redex のルートを更新するときに、$a$ から派生するスパイン内のすべてのノードのロックを解除する必要があります。
したがって、遷移規則は次の様になります。
...
\begin{tabular} {|l r l l l l r r r l|} \hline
	                  & $\langle$ & $h\left[
			\begin{array}{l}
				a_1' : \texttt{NGlobal} ~ n ~ c               \\
				a_2' : \texttt{*NAp} ~ a_1' ~ a_1             \\
				\cdots                                        \\
				a_{n-1}' : \texttt{*NAp} ~ a_{n-2}' ~ a_{n-2} \\
				a_n : \texttt{*NAp} ~ a_{n-1}' ~ a_{n-1}
			\end{array}
	\right]$          & $m$       & $t$      & $\rangle$ & $\langle$ & \texttt{Update} $n$ \texttt{:} $i$ & $a$ \texttt{:} $a_1$ \texttt{:} $\ldots$ \texttt{:} $a_n$ \texttt{:} $s$ & $\rangle$ \\
	$\Longrightarrow$ & $\langle$ & $h\left[
			\begin{array}{l}
				a_1' : \texttt{NGlobal} ~ n ~ c              \\
				a_2' : \texttt{NAp} ~ a_1' ~ a_1             \\
				\cdots                                       \\
				...
				a_n : \texttt{NInd} ~ a
			\end{array}
	\right]$          & $m$       & $t$      & $\rangle$ & $\langle$ & $i$                                & $a_1$ \texttt{:} $\ldots$ \texttt{:} $a_n$ \texttt{:} $s$                & $\rangle$ \\ \hline
\end{tabular}
\vskip\baselineskip

\begin{description}
	\item[演習 5.9] 遷移関数 \texttt{unwind} と \texttt{update} の定義を変更します。
		コードでは \texttt{lock} 関数と \texttt{unlock} 関数を使用する必要があります。
		また、スタックを再配置するときにロックされたノードを「調べる」必要があるため、\texttt{getArg} は次のようになります。
		\begin{verbatim}
					getArg (NLAp a1 a2) = a2
				  \end{verbatim}
	\item[演習 5.10] あなたのマシンで次のプログラムを実行してみてください。
		\begin{verbatim}
				     main = twice' (twice' (twice' (S K K))) 3
				     twice' f x = par f (f x)
				   \end{verbatim}
	\item[演習 5.11] 分割統治プログラムは、実行中の各プロセスで30命令ごとに \texttt{par} 命令を実行します。
		プロセスは終了しません。
		宇宙の電子1つにつき1つのタスクが実行されるまでに、シミュレートされたクロックティックが何回経過しますか?
		(ヒント: 宇宙には約 $10^{85}$ 個の電子があります。)
	\item[演習 5.12] プロセッサのコストが $\pounds 0.01$ の場合、誰もそのマシンを購入できなくなるまで、演習 5.11 のプログラムはどのくらい実行できますか?
		(ヒント: 米国の連邦予算は約 $\pounds 5 \times 10^{12}$ です。)
\end{description}
\newpage

\section{Mark 3: 現実的な並列Gマシン}

演習 5.11 と 5.12 で示したように、現実世界では並列処理の量には物理的および経済的な制限があります。
使用している並列処理モデルはあまり現実的ではありません。
各並列タスクを実行するために新しいプロセッサを作成していますが、現実世界ではすぐにリソースが不足します。

\subsection{スケジューリングポリシー}

より現実的なモデルでは、並列処理の量をハードウェアによって提供される量に制限する必要があります。
これは、一度に実行できるプロセッサの数に上限を設定することで簡単に実現できます。
その結果、タスクを実行するために使用できるプロセッサがない場合、タスクは変更されません。
\vskip\baselineskip

タスクよりもプロセッサの数が多い場合、一部のプロセッサはアイドル状態になります。
一方、逆の場合は、次にどのタスクを実行するかを決定する問題に直面します。
この決定は、\textbf{スケジューリングポリシー}と呼ばれます。

\subsection{保守的な並列性と投機的な並列性}

いくつかのタスクは他のタスクよりも重要になります。
タスクは、次の2つのグループに分類できます:

\begin{itemize}
	\item 結果が確実に必要になるタスク
	\item 結果が必要になる可能性があるタスク。
\end{itemize}

最初のカテゴリのタスクを\textbf{保守的タスク}と呼び、2番目のカテゴリのタスクを\textbf{投機的タスク}と呼びます。
\vskip\baselineskip

投機的並列タスクを許可することを選択した場合、タスクのスケジュール設定における優先順位の問題に対処する必要があります。
つまり、タスクを重要度順にランク付けする必要があります。
また、同じ優先順位を持つ異なるタスクに同じ計算時間を許可する必要があります。
これが望ましい理由を理解するには、判別式 \texttt{e0} の評価と並行してケース式の 2 つの分岐 (\texttt{e1} と \texttt{e2}) を評価することを検討してください。
...
\texttt{e0} の評価が完了するまで、\texttt{e1} と \texttt{e2} のどちらが必要になるかはわかりません。
したがって、それぞれを同量評価するようにするのが理にかなっています。
このようなスケジューリングポリシーは、\textbf{公平なスケジューリング}と呼ばれます。
この例では、\texttt{e0} が完了すると、タスクの 1 つ (\texttt{e1} または \texttt{e2}) が必要になり、
もう 1 つは強制終了する必要があることに注意してください。
したがって、タスクの実行中にタスクの優先順位を調整する必要があります。
\vskip\baselineskip

この本では、投機的並列処理に適したマシンを実装する試みは行いません。
これは「難しい問題」であるという言い訳をして、この問題はそのままにしておきます。
今後は、 \texttt{par} プリミティブの使用はすべて、保守的な並列処理のみを生み出す状況で発生するものと想定します。
\vskip\baselineskip

Mark 3 並列Gマシンでは、マシン内のアクティブなタスクの数は限られています。
これらはマシン内のプロセッサによって実行されます。
プロセッサの数は固定されています。
この固定数は \texttt{machineSize} で、現在は \texttt{4} に設定されています。
...
評価ツールの主な変更点は \texttt{steps} 関数にあります。これは、作成されたすべてのタスクをマシンに追加するだけではありません。
代わりに、\texttt{scheduler} を使用して、続行できないタスクの代替タスクを選択します。

\begin{enumerate}
	\item まず、グローバル状態コンポーネントからタスクプール内のスパークを抽出します。
	\item 次に、スパークに対して新しいタスクが作成され、すでに実行中のタスクに追加されます。
	\item 次に、\texttt{scheduler} 関数が実行するタスクを選択します。
\end{enumerate}

\texttt{steps} をコード化する方法は次のとおりです。
...
スケジューリングポリシーは非常にシンプルです。
最初の \texttt{machineSize} タスクを選択して実行します。
これらのタスクは、スケジューリングキューの最後に配置されます。
このスケジューリングポリシーは通常、\textbf{ラウンドロビン}スケジューリングポリシーと呼ばれます。
...
\begin{description}
	\item[演習 5.13] 実行されたタスクが次のステップのスケジュールキューの最後に配置されていない場合、何が起こるでしょうか。
		(ヒント: 試してみてください!)
	\item[演習 5.14] 改善できる点の1つは、アイドル状態のプロセッサがある場合にのみ、スパークプールからタスクを作成することです。
		これを行うには、\texttt{steps} を変更します。
	\item[演習 5.15] もう 1 つの改善点は、続行可能なタスクのみをスケジュールすることです。
		現時点では、すでに WHNF にあるノードを評価するタスクをスケジュールします。
		ロックされたノードをアンワインドしようとしているためにブロックされているタスクもスケジュールします。
		これが発生しないように \texttt{scheduler} を変更します。
	\item[演習 5.16] 他のスケジューリング戦略の使用を調査します。
		たとえば、タスクプールの最後のタスクをスケジュールしてみます。
		スケジューリング戦略によってプログラムの実行時間に違いが生じますか?
\end{description}

\newpage

\section{Mark 4: ブロックを処理するためのより良い方法}

これまで、ブロックされたタスクはマシンのローカル状態に残し、\texttt{scheduler} 関数で実行可能なタスクを選択する必要がありました。
つまり、\texttt{scheduler} 関数は、実行可能なタスクを見つけるまでに、かなりの数のブロックされたタスクをスキップしなければならない可能性があります。
\vskip\baselineskip

これよりも良い方法があります!
ブロックされたタスクを、ブロックの原因となったノードにアタッチする方がはるかに良いでしょう。
ロックされたノードは任意の数のタスクをブロックする可能性があるため、ロックされたノードにタスクのリストを配置できるようにする必要があります。
これを\textbf{保留リスト}と呼びます。
\vskip\baselineskip
...
私たちが採用した解決策は、タスクを \texttt{emptyTask} に置き換えることです。
...
したがって、\texttt{Unwind} には2つの新しい遷移規則が必要です。
まず、ロックされた関数適用ノードの遷移規則から始めます。
この遷移では、タスクがノードの保留リストに配置され、タスクが \texttt{emptyTask} に置き換えられます。
\vskip\baselineskip

(5.5) \\
\begin{tabular} {|l r l l l l r r r l|} \hline
	                  & $\langle$ & $h[a : \texttt{*NAp} ~ a_1 ~ a_2 ~ pl]$                                                                & $m$ & $t$ & $\rangle$ & $\langle$ & [\texttt{Unwind}]  & $a$ \texttt{:} $s$ & $\rangle$ \\
	$\Longrightarrow$ & $\langle$ & $h[a : \texttt{*NAp} ~ a_1 ~ a_2 ~ \langle [\texttt{Unwind}], ~ a \texttt{:} s \rangle \texttt{:} pl]$ & $m$ & $t$ & $\rangle$ &           & \texttt{emptyTask} &                    &           \\ \hline
\end{tabular}
\vskip\baselineskip

ロックされたグローバルノードのルールも同様です。
タスクはノードの保留リストに配置され、それ自体が \texttt{emptyTask} に置き換えられます。
\vskip\baselineskip

(5.6) \\
\begin{tabular} {|l r l l l l r r r l|} \hline
	                  & $\langle$ & $h[a : \texttt{*NGlobal} ~ 0 ~ c ~ pl]$                                                                & $m$ & $t$ & $\rangle$ & $\langle$ & [\texttt{Unwind}]  & $a$ \texttt{:} $s$ & $\rangle$ \\
	$\Longrightarrow$ & $\langle$ & $h[a : \texttt{*NGlobal} ~ 0 ~ c ~ \langle [\texttt{Unwind}], ~ a \texttt{:} s \rangle \texttt{:} pl]$ & $m$ & $t$ & $\rangle$ &           & \texttt{emptyTask} &                    &           \\ \hline
\end{tabular}
\vskip\baselineskip

\begin{description}
	\item[演習 5.21] \texttt{Unwind} 関数を変更して、\texttt{Unwind} 命令の新しい遷移を実装します。
		\texttt{getArg} 関数も作成する必要があります。
		\begin{verbatim}
		    getArg (NLAp a1 a2 pl) = a2
	    \end{verbatim}
	\item[演習 5.22] 実行されていないタスクをスパークプールに配置するように \texttt{scheduler} 関数を変更します。
	\item[演習 5.23] \texttt{doAdmin} 関数を変更して、ローカル状態から \texttt{emptyTask} を除外します。
\end{description}
...
私たちがカバーしていない最後の領域の1つは、分散メモリ並列マシンの領域です。
...
\begin{itemize}
	...
	\item ローカル関数定義がないようにプログラムを変換します。
	      代わりに、すべての関数がスーパーコンビネータとして定義されます。
\end{itemize}

この本では、常に2番目のアプローチが取られると想定してきました。

この章は、\textbf{完全な遅延評価}の概念を紹介する適切なポイントでもあります。
ここでも、関数型言語のこの望ましい最適化は、プログラム変換を使用して実現されます。

\section{\texttt{expr} データ型の改善}
...
残念ながら、そこで定義されているデータ型 (\texttt{CoreExpr}、\texttt{CoreProgram} など) は、
この章でのニーズを満たすには柔軟性が不十分なので、まずこの問題に取り組みます。
多くのコンパイラパスは抽象構文木に情報を追加するため、この情報を表す体系的な方法が必要です。
この種の情報を生成する分析の例としては、自由変数分析、バインディングレベル分析、型推論、厳密性分析、共有分析などがあります。
\vskip\baselineskip

このような情報を追加する最も明白な方法は、\texttt{Expr} データ型に注釈用の新しいコンストラクタを追加することです。
...
これにより、構文ツリー全体に注釈を自由に表示できるようになり、非常に柔軟に見えます。
実際には、2つの大きな欠点があります。

\begin{itemize}
	\item 先ほど説明した形式で注釈情報を\textbf{追加する}のは簡単ですが、
	      前のパスで配置された情報を\textbf{使用する}コンパイラパスを記述するのは実に厄介です。
	      たとえば、パスが、前のパスでツリーの各ノードに残された自由変数情報を使用したいとします。
	      おそらくこの情報は各ノードのすぐ上に添付されますが、データ型によって各ノードの上に複数の注釈ノードが表示されることが許可され、
	      さらに悪いことに、自由変数注釈がまったく存在しない(または1つ以上する)可能性があります。
	      \vskip\baselineskip

	      プログラマが、各ツリーノードの上に1つの注釈ノードがあり、それが自由変数注釈であることを証明する準備ができている場合でも、
	      実装は注釈を抽出するときにパターンマッチングを実行してこれらのアサーションをチェックします。
	      これらの問題、つまりチェックできないプログラマのアサーションの必要性と実装のオーバーヘッドは、
	      注釈が付けられたすべてのツリーが、どの注釈が存在するかについて何も言わない、
	      あまり情報のない型 \texttt{Expr} を持っているという事実に直接起因します。
	\item 2つ目の大きな問題は、さらに実験を進めると、2つの異なる形式の注釈が必要であることがわかったことです。
	      1つ目は上記のように式に注釈を付けますが、2つ目は変数のバインディング出現に注釈を付けます。
	      つまり、\texttt{let(rec)} 定義の左辺の出現と、ラムダ抽象化または \texttt{case} 式内のバインドされた変数です。
	      これらの出現を\textbf{バインダ}と呼びます。
	      バインダに注釈を付ける必要がある例として、型推論が挙げられます。
	      型推論では、コンパイラが各バインダと各部分式の型を推論します。
	      \vskip\baselineskip

	      式注釈を使用してバインダに注釈を付けることもできますが、これは扱いにくく不便です。
\end{itemize}

簡単な解決策があるため、最初に2番目の問題に対処します。
セクション1.3から、\texttt{Expr} 型がバインダの型に関して\textbf{パラメータ化された}ことを思い出してください。
ここでは、リマインダーとしてその定義を繰り返します。
...
\texttt{CoreExpr} 型は、バインダが \texttt{Name} 型の特殊な形式の \texttt{Expr} です。
これは、型シノニムを使用して表現されます(セクション \ref{sec_1_3} からも繰り返されます）。

\begin{verbatim}
type CoreExpr = Expr Name
\end{verbatim}

パラメータ化された \texttt{Expr} の利点は、他の特殊な形式を定義できることです。
たとえば、\texttt{typedExpr} は、バインダが型で注釈された名前であるデータ型です。

\begin{verbatim}
  typedExpr = expr (name, typeExpr)
\end{verbatim}

ここで、\texttt{typeExpr} は型式を表すデータ型です。
\vskip\baselineskip

式の注釈に戻ると、注釈型に関して式のデータ型をパラメーター化することにより、同じ手法を再利用できます。
ツリーのすべてのノードに注釈を付けたいので、1つの可能性は、注釈情報を使用してすべてのコンストラクタに追加のフィールドを追加することです。
たとえば、ルートノードでケース分析を実行せずに特定の式の上部に自由変数情報を抽出したい場合、これは不便です。
これは、次のアイデアにつながります。
...
\texttt{AnnExpr} と \texttt{AnnExpr'} 間の相互再帰によって、ツリー内のすべてのノードがアノテーションを持つことが保証されていることに注目してください。
式が持つアノテーションの種類は、式の型に明示されます。
例えば、自由変数でアノテーションされた式の型は \texttt{AnnExpr Name (Set Name))} です。
\vskip\baselineskip

\texttt{AnnExpr'} と \texttt{Expr} が本質的に同一のコンストラクタ群を二つ定義しなければならないというのは実に厄介な問題です。
Hindley-Milner 型システムでは、この問題を回避する方法はなさそうです。
\texttt{Expr a} は \texttt{AnnExpr a b} とほぼ同型なので、\texttt{Expr} 型を完全に廃止することも可能ですが、
そうしないことにしたのには2つの理由があります。
第一に、後者(\texttt{AnnExpr a b})は \texttt{(() \footnote{\texttt{()} はユニット。} , $\bot$ )} と $\bot$ を区別しますが、
前者(\texttt{Expr a})は区別しないため、この 2 つの型は完全に同型ではありません。
第二に (そしてより深刻なことですが)、\texttt{AnnExpr a b} 型のツリーを構築したりパターンマッチングしたりする際に、
すべての \texttt{()} を書くのは非常に面倒です。
\vskip\baselineskip

これで、中心となるデータ型の開発は完了です。
これまでの議論を通して、現代の関数型プログラミング言語が提供する代数的データ型の長所と短所がいくつか明らかになりました。

\begin{description}
	\item[演習 6.1] 1.5節で定義した現在の整形出力関数 \texttt{pprint} は、\texttt{coreProgram} のみを出力できます。
		ラムダリフターの中間段階を出力するには、\texttt{program *} 型の値を表示できる関数 \texttt{pprintGen} が必要です。
		('\texttt{Gen}' は 'generic' の略です。)
		\texttt{pprintGen} には、バインダの出力方法を指定するための追加の引数が必要です。
		\begin{verbatim}
          pprintGen :: (String -> Iseq)  -- function from binders to iseq
                       -> Program Name   -- the program to be formatted
                       -> [Char]         -- result string
        \end{verbatim}
		例えば、\texttt{pprintGen} を記述したら、それを使って \texttt{pprint} を定義できます。
		\begin{verbatim}
          pprint prog = pprintGen iStr prog
        \end{verbatim}
		\texttt{pprintGen} とそれに関連する関数 \texttt{pprExprGen} などの定義を記述しなさい。
	\item[演習 6.2] \texttt{annProgram * **} 型の値を出力するためにも同様の処理を行います。
		ここでは、バインダのフォーマット用とアノテーションのフォーマット用の2つの追加引数が必要になります。
		\begin{verbatim}
          pprintAnn :: (a -> Iseq)        -- function from binders to iseq
                       ...
                       -> [Char]          -- result string
        \end{verbatim}
		\texttt{pprintAnn} とそれに関連する補助関数の定義を記述しなさい。
\end{description}

\section{Mark 1: シンプルなラムダリフター}

レキシカルスコープのプログラミング言語の実装は、関数や手続きが\textbf{自由変数}を持つ可能性があるという事実に対処する必要があります。
何らかの方法で自由変数を削除しない限り、環境ベースの実装ではリンクされた環境フレームを操作する必要があり、
簡約ベースのシステムは置換時に名前変更を行う必要があるため、大幅に複雑になります。
特にグラフ簡約の実装において、これらの問題を回避する一般的な方法は、
ラムダリフティングと呼ばれる変換によって関数定義からすべての自由変数を削除することです。
ラムダリフティングという用語は [Johnsson 1985] によって造られましたが、この変換は [Hughes 1983] によって独自に開発されました。
\vskip\baselineskip

ここでの文脈では、ラムダリフティングはコア言語のプログラムを、埋め込まれたラムダ抽象化のない同等のプログラムに変換します。
簡単な例として、次のプログラムを考えてみましょう。
...
\texttt{\textbackslash y} 抽象化は、\texttt{x} を追加パラメータとして受け取り、
その本体が問題となっている抽象化である新しいスーパーコンビネータ \texttt{\$g} を定義し、
\texttt{\textbackslash y} 抽象化を \texttt{\$g} の適用に置き換えることで削除できます。
これにより、次のスーパーコンビネータ定義のセットが得られます。
...
なぜ \texttt{x} だけを \texttt{\$g} の追加パラメータにすることにしたのでしょうか?
\texttt{x} は抽象化 \texttt{\textbackslash y. x*x + y} の\textbf{自由変数}であるため、このようになりました。

\begin{quote}
	\textbf{定義} 式 $e$ 内の変数 $v$ の出現が $e$ 内のその変数を囲むラムダ式または \texttt{let(rec)} 式によって束縛されていない場合、
	その出現は $e$ 内で\textbf{自由}であると言われる。
\end{quote}

一方、\texttt{y} は \texttt{(\textbackslash y. x*x + y)} では自由ではありません。
これは、\texttt{y} の出現が、それを束縛するラムダ抽象が囲むスコープ内にあるためです。
\vskip\baselineskip

再帰が絡んでも、問題はそれほど複雑ではありません。
\texttt{g} が再帰的であると仮定すると、次のようになります。
...
これで、\texttt{x} と \texttt{g} は両方とも \texttt{\textbackslash y} 抽象化内で自由になったため、
ラムダリフターはそれらを両方とも \texttt{\$g} の追加パラメーターにして、次のスーパーコンビネータのセットを生成します。
...
\texttt{g} の定義は依然として再帰的ですが、ラムダリフターによって局所的なラムダ抽象化が排除されていることに注意してください。
このプログラムは、ほとんどのコンパイラバックエンド、特に本書で紹介するすべての抽象マシンによって直接実装できるようになりました。
\vskip\baselineskip

最後にもう一つ補足しておきます。他のトップレベル関数を追加の引数として扱う必要はありません。
...
ここでは、\texttt{h} を \texttt{\textbackslash y} 抽象化の自由変数として扱いたくありません。
なぜなら、\texttt{h} は新しい \texttt{\$g} スーパーコンビネータの本体から直接参照できる定数だからです。
もちろん、\texttt{+} 関数と \texttt{*} 関数にも同じことが当てはまります。
つまり、スーパーコンビネータの引数、およびラムダ抽象化や \texttt{let(rec)} 式で束縛された変数だけが、自由変数の候補となります。
\vskip\baselineskip

ついでに言えば、レキシカルスコープの問題は関数型言語に限った話ではありません。
例えば、Pascalでは関数を別の関数内でローカルに宣言することが可能で、その関数は外側のスコープによって束縛された自由変数を持つことができます。
一方、C言語ではこのようなローカルな関数定義は許可されていません。
副作用がなければ、ローカルな関数定義をグローバルなものにするのは簡単です。
必要なのは、自由変数を追加のパラメータとして追加し、すべての呼び出しにこれらのパラメータを追加することだけです。
まさにこれがラムダリフティングの目的です。

\subsection{シンプルなラムダリフターの実装}
...
\begin{itemize}
	...
	\item ここで、プログラムを走査し、各変数に一意の名前を付けます。
	      ...
	      副作用として、プログラム内の他のすべての名前もユニークになりますが、これは問題ではなく、後で役立つことがわかります。
	      \begin{verbatim}
            rename :: CoreProgram -> CoreProgram
          \end{verbatim}
	\item 最後に、\texttt{collectSCs} はすべてのスーパーコンビネータの定義を1つのリストに収集し、プログラムの最上位レベルに配置します。
	      \begin{verbatim}
            collectSCs :: CoreProgram -> CoreProgram
          \end{verbatim}
\end{itemize}

ラムダ リフター自体は、次の3つの関数を合成したものです。

\begin{verbatim}
  lambdaLift = collectSCs . rename . abstract . freeVars
\end{verbatim}

何が起こっているかを簡単に確認できるように、パーサとプリンタを統合する関数 \texttt{runS} ('\texttt{S}' は 'simple' の略) を定義します。

\begin{verbatim}
  runS = pprint . lambdaLift . parse
\end{verbatim}

もちろん、すべての作業を1つのパスで実行することも可能ですが、パスを分離することで提供されるモジュール性には、いくつかの利点があります。
個々のパスは理解しやすくなり、パスは再利用可能になり(たとえば、以下では \texttt{freeVars} を再利用します)、
モジュール性によってアルゴリズムを多少変更しやすくなります。
\vskip\baselineskip

最後の点の例として、一部のコンパイラは \texttt{collectSCs} パスを省略することでより良いコードを生成できます。
これは、スーパーコンビネータが適用されるコンテキストについてより多くの情報がわかるためです。 [Peyton Jones 1991]
例えば、\texttt{abstract} パスによって生成される可能性のある次の式を考えてみましょう。
...
ここで \texttt{abstract} は、自由変数である \texttt{v} を \texttt{\textbackslash x} 抽象化から削除しています。
\footnote{スーパーコンビネータに名前を付けるために \texttt{abstract} が導入する \texttt{let} 式は無視します。}
スーパーコンビネータをそのコンテキストから独立してコンパイルするのではなく、コンパイラは \texttt{f} のクロージャを構築し、
そのコードでクロージャから \texttt{v} に直接アクセスし、スタックから \texttt{x} にアクセスすることができます。
したがって、\texttt{f} の呼び出しで \texttt{v} をスタックに移動する必要はありません。
自由変数が多ければ多いほど、この利点は大きくなります。
また、定義がローカルなものであるため、\texttt{f} の呼び出しの効率が低下することもありません。
コンパイラは \texttt{f} の束縛を認識し、そのコードに直接ジャンプできます。
\vskip\baselineskip
...
\texttt{let(rec)} 式の等式は、かなり複雑な構造になっていますが、非常に単純です。
本体に渡されるスコープ内のローカル変数は \texttt{body\_lv} です。
各右辺に渡されるローカル変数の集合は \texttt{rhs\_lv} です。
次に、各右辺に自由変数集合 \texttt{rhss'} をアノテーションします。
これにより、アノテーション付き定義 \texttt{defns'} を構築できます。
\texttt{let(rec)} のアノテーション付き本体は \texttt{body'} です。
定義の自由変数は \texttt{defnsFree} として計算され、本体の自由変数は \texttt{bodyFree} として計算されます。
...
\texttt{defns'} の定義にある関数 \texttt{zip2} は、2つのリストを受け取り、引数リストの対応する要素のペアからなるリストを返す標準関数です。
\texttt{setUnion}、\texttt{setSubtraction} などの集合演算は utilities モジュールで定義されており、そのインターフェースは付録 A.4 に示されています。
\vskip\baselineskip

\texttt{case} とコンストラクタ式の処理は保留します。
...
\texttt{freeVarsOf} と \texttt{freeVarsOf\_alt} は単純な補助関数です。
...
いつものように、ほとんどの作業を実行する補助関数 \texttt{abstract\_e} を定義します。

\begin{verbatim}
  abstract_e :: AnnExpr Name (Set Name) -> CoreExpr
\end{verbatim}

これは、自由変数情報で注釈が付けられた式を受け取り、各ラムダ抽象化を自由変数に適用した新しい抽象化に置き換えた式を返します。
最初の4つのケースについてはあまり説明する必要はありません。これらは単に各式を再帰的に抽象化するだけです。
...
関数\texttt{foldll}は標準関数で、付録 A.5 で定義されています。
2項関数 $\oplus$ 、値 $b$、リスト $xs = \lbrack x_1, \ldots, x_n \rbrack$ が与えられると、
\texttt{foldll} $b$ $xs$ は $(\ldots ((b \oplus x_1) \oplus x_2) \oplus \ldots x_n )$ を計算します。
自由変数情報は不要になったため、パスによって破棄されることに注意してください。
\vskip\baselineskip
...
\section{Mark 2: シンプルなラムダリフターの改良}

これで、単純なラムダリフターの定義は完了です。
次に、いくつかの簡単な改良について検討します。

\subsection{シンプルな拡張機能}
...
\section{Mark 3: ジョンソンスタイルのラムダリフティング}

ラムダリフティング技法には興味深い変種があり、[Johnsson 1985] によって発見されました。
現在の技法には、引数として渡される関数への呼び出しが多く含まれるプログラムが生成されてしまうという小さな問題があります。
例えば、6.3節の再帰例を考えてみましょう。
...
\texttt{\$g} が引数 \texttt{g} を呼び出すことに注意してください。
実装によっては、次のように \texttt{\$g} を直接再帰的に実行する方が効率的です。
...
内部の \texttt{letrec} は完全に消え、スーパーコンビネータ $g$ は直接再帰的になりました。
\vskip\baselineskip
...
ここで、\texttt{g} は \texttt{h} を呼び出し、変数 \texttt{x} に言及する関数です。
同様に、\texttt{h} は \texttt{g} を呼び出し、変数 \texttt{y} に言及します。
最初のステップは、定義を次のように変換することです。
...
この変換は\textbf{抽象化ステップ}と呼ばれていますが、少し複雑です。
以下の処理が行われます。

\begin{itemize}
	\item \texttt{letrec} の右辺の自由変数、つまり \texttt{g}、\texttt{h}、\texttt{x}、\texttt{y} を取ります。
	\item バインドされている変数( \texttt{g} と \texttt{h} )を除外して、\texttt{x} と \texttt{y} だけを残します。
	\item これらの変数を各右辺の追加引数にします。
	\item そして、\texttt{g} をすべて \texttt{(g x y)} に置き換え、\texttt{h} についても同様です。
\end{itemize}

\texttt{y} は \texttt{g} の右辺に直接出現しませんが、\texttt{g} が \texttt{h} に渡すために \texttt{y} を必要とするため、
\texttt{y} を \texttt{g} の追加パラメータにすることが重要です。
一般に、相互再帰グループの各メンバーは、\textbf{すべての}メンバーの自由変数をまとめて追加引数として取る必要があります。
\vskip\baselineskip

ここで必要なのは、\texttt{g} と \texttt{h} の定義をトップレベルにフロートして、次のコードを残すだけです。
...
最後にもう1点。
このプロセスを実行する前に、すべてのバインダーが一意であることが重要です。
そうでない場合、2つの理由で名前の衝突が発生する可能性があります。
明らかなのは、2つのスーパーコンビネータが同じ名前を持つことです。
あまり明らかではない例として、同じ例の次のバリエーションを示します。
...
すると、\texttt{h} は引数に \texttt{f} と同じ名前を使用するようになり、
\texttt{g} の自由変数 \texttt{x} を \texttt{h} の追加引数にしようとすると問題が発生します。
全体として、作業を開始する前にプログラムの名前を変更する方がはるかに簡単です。
...
\begin{itemize}
	\item 最初のパスでは、すべてのバインダーが一意になるようにプログラムの名前を変更します。
	      この目的のために、\texttt{rename} 関数を再利用できます。
	\item 次に、既存の関数 \texttt{freeVars} を使用して、プログラムに自由変数情報を注釈付けします。
	\item ここで、上で説明した主要な抽象化のステップに進みます。
	      \begin{verbatim}
	      	abstractJ :: AnnProgram Name (Set Name) -> CoreProgram
	      \end{verbatim}
	\item 最後に、\texttt{collectSCs} を使用してスーパーコンビネータを収集できます。
\end{itemize}
...
\subsection{関数内の自由変数の抽象化}

必要な新しい関数は \texttt{abstractJ} だけです。
抽象化プロセスは、処理が進むにつれて $g$ を $g ~ v_1 \ldots v_n$ に置き換えます。
ここで $g$ は新しいスーパーコンビネータの1つであり、$v_1, \ldots, v_n$ はその宣言グループの自由変数です。
したがって、補助関数 \texttt{abstractJ\_e} は、各スーパーコンビネータ $g$ をそのグループ $v_1, \ldots, v_n$ の自由変数にマッピングする環境を受け取る必要があります。
...
公平に言えば、最初の引数は \texttt{ASSOC Name CoreExpr} 型になるように見えますが、後で説明するように、環境を別の方法でも使用する必要があり、それがここで提案する型につながります。
\vskip \baselineskip

各トップレベルの定義に \texttt{abstractJ\_e} を適用することで、\texttt{abstractJ} を \texttt{abstractJ\_e} で簡単に定義できます。
...
さて、\texttt{abstractJ\_e} について見てみましょう。
定数と関数適用のケースは簡単です。
...
変数 $g$ に遭遇すると、環境内でその変数を検索し、変数のリスト $v_1, \ldots, v_n$ を取得します。
その後、適用結果 $g ~ v_1 \ldots v_n$ を返します。
$g$ が環境に現れない場合は、環境検索から空のリストを返します。
したがって、構築する「適用結果」は単に変数 $g$ になります。
...
\texttt{\textbackslash x} 抽象化は \texttt{let(rec)} 定義の右辺ではないので、単純なラムダリフターの場合と同じように扱います(\texttt{abstract} の \texttt{ELam} ケースを参照)。
\vskip \baselineskip

重要な違いが1つだけあります。
\texttt{abstractJ\_e} は式に対して同時に置換を実行するため、自由変数情報は置換後の状態を反映しません。
むしろ、結果の中でどの変数が自由であるかを調べるために、自由変数集合に対しても置換を実行する必要があります。
これは、このセクションの最後で定義されている関数 \texttt{actualFreeList} によって行われます。
自由変数情報に対してこの操作を実行する必要性が、環境表現の選択を導きました。
...
最後に、\texttt{let(rec)} 式を扱います。
ラムダ抽象に束縛された各変数はスーパーコンビネータに変換されますが、ラムダ抽象に束縛されていない他の変数はスーパーコンビネータに変換されません。
したがって、これら2種類の定義を別々に扱う必要があり、それぞれ「関数定義」と「変数定義」と呼びます。
...
関数定義と変数定義を分離したので、関数から抽象化する変数の集合を計算できます。
関数定義の自由変数の和集合を取り、この集合から束縛される関数名を取り除き、\texttt{actualFreeList} (\texttt{ELam} 定義式と同じ理由)を使用して結果を取得します。
...
次に、\texttt{let(rec)} の右辺と本体で使用される新しい環境を計算します。
...
最後に、適切な環境で \texttt{abstractJ\_e} を再帰的に使用して、新しい関数定義、変数定義、および本体を計算します。
...
関数 \texttt{actualFreeList} は環境と自由変数の集合を受け取り、その集合に環境を適用し、置換後の自由変数のリスト(重複なし)を返します。
...
関数 \texttt{isALam} は \texttt{ALam} コンストラクタを識別します。
...
\begin{description}
	\item[演習 6.7] 関数 \texttt{abstractJ\_e} に \texttt{case} 式のケースを追加します。
\end{description}

\subsection{難しい点 \dag}

\texttt{letrec} に関数定義と変数定義が混在している場合、設計したラムダリフターによって冗長なパラメータが導入される可能性があります。
例えば、次の定義を考えてみましょう。
...
\texttt{g} / \texttt{h} グループの自由変数は \texttt{x}、\texttt{y}、\texttt{k} なので、次のように変換します。
...
ここで、追加のパラメータ $x$ は実際には $h$ では使用されません。
したがって、$g$ と $h$ のより良い定義は次のようになります。
...
\begin{description}
	\item[演習 6.8 $\dag$] このより洗練された変換を実行するには、\texttt{abstractJ} を修正してください。
		警告: これはかなり難しい作業です。
\end{description}

\newpage

\section{Mark 4: 完全な遅延処理パスの分離}

ここで、関数型プログラムの重要な特性である\textbf{完全遅延性}に注目します。
完全遅延性に関するこれまでの説明では、常にラムダリフティングが関連付けられており、「完全遅延ラムダリフティング」と説明されていますが、これはかなり複雑なプロセスであることがわかります。
[Hughes 1983] はアルゴリズムを示していますが、非常に微妙で、\texttt{let(rec)} 式を扱っていません。
一方、[Peyton Jones 1987] は \texttt{let(rec)} 式を扱っていますが、その説明は非公式なもので、アルゴリズムは示されていません。
\vskip\baselineskip

このセクションでは、完全な遅延処理とラムダ式による処理を明確に分離する方法を示します。
これは、\texttt{let} 式を用いた変換によって行われます。
ある方法で単純化したものが、別の方法で複雑化しただけだと誤解されないよう、\texttt{let(rec)} 式を使用せずに完全な遅延処理のラムダ式処理を実行すると、
予期せぬ遅延処理の損失が発生するリスクがあることも示します。
さらに、ほとんどのコンパイラでは、\texttt{let(rec)} 式に対して、同等のラムダ式よりもはるかに効率的なコードを後の段階で生成できます。

\subsection{完全な遅延処理のレビュー}

まず、完全遅延処理の概念を簡単に確認してみましょう。
6.3節で示した例をもう一度考えてみましょう。
...
\texttt{f} の本体には \texttt{g} と \texttt{\$g} が2回呼び出されています。
しかし \texttt{(\$g x)} は簡約可能な式ではないため、\texttt{x*x} は2回計算されます。
しかし、\texttt{x} は \texttt{f} の本体で固定されているため、一部の作業が重複しています。
\texttt{x*x} の計算を2回の \texttt{\$g} の呼び出しで共有する方が適切です。
これは次のように実現できます。
\texttt{x} を \texttt{\$g} の引数にする代わりに、\texttt{x*x} を引数にします。
...
(\texttt{main} の定義は変更されないため、以降は省略します。）
したがって、完全遅延評価ラムダリフターは、ラムダ抽象の各自由変数ではなく、各最大自由部分式を、対応するスーパーコンビネータの引数に変換します。
ラムダ抽象の最大自由式(またはMFE)とは、抽象化によって束縛される変数の出現を含まない式であり、この特性を持つより大きな式の部分式ではありません。
\vskip\baselineskip

完全な遅延評価は、ループ不変式をループの外側に移動する操作に正確に対応し、ループの反復ごとに1回ではなく、開始時に1回だけ計算されます。

完全な遅延性は「実際の」プログラムにとってどれほど重要なのでしょうか？
この問題については、まだ本格的な研究は行われていませんが、私たちはそうする予定です。
しかしながら、Holst による最近の研究は、完全な遅延性の重要性は当初考えられていたよりも大きい可能性があることを示唆しています [Holst 1990]。
彼は、完全な遅延性の効果を自動的に高める変換を実行する方法を示しており、得られる最適化は部分評価 [Jones et al. 1989] によって得られる最適化に匹敵しますが、その労力ははるかに少なくなります。

\subsection{\texttt{let(rec)} 式が存在する場合の完全遅延評価ラムダリフティング}

前のセクションで概説したように、完全に遅延評価を行うラムダリフターを書くのは驚くほど難しいです。
\texttt{let(rec)} 式を含む私たちの言語は、新しい言語構成要素を導入することで、この難しさをさらに悪化させているようです。
例えば、実行例の \texttt{g} の定義が
少し複雑で、次のようになっていたとします。
...
ここで、部分式 \texttt{x*x} は \texttt{\textbackslash y} 抽象化の MFE ですが、部分式 \texttt{z*z} は \texttt{z} が \texttt{\textbackslash y} 抽象化内に束縛されているため、MFE ではありません。
しかし、\texttt{p} が \texttt{x} のみ(間接的ではありますが)に依存していることは明らかなので、\texttt{z*z} が一度だけ計算されるようにする必要があります。
\vskip\baselineskip

完全遅延ラムダリフタは、\texttt{let} 式がラムダ適用として記述されている場合、これを検知しますか？
いいえ、検知しません。
\texttt{g} の定義は次のようになります。

\begin{verbatim}
	g = \y. (\z. (\p. p+y) (z*z)) (x*x)
\end{verbatim}

ここで、\texttt{x*x} は以前と同様に自由ですが、\texttt{z*z} は自由ではありません。
言い換えれば、\textbf{コンパイラが \texttt{let(rec)} 式を特別扱いしない場合、プログラマが当然期待する完全な遅延表現が失われる可能性があります。}
\vskip\baselineskip

幸いなことに、\texttt{let(rec)} 式を扱う簡単な方法があります。
[Peyton Jones 1987、第15章] で説明されているように、\texttt{let(rec)} 定義を、それが自由であるラムダ抽象の外側まで外側に「浮上」させるのです。
例えば、\texttt{g} の定義を次のように変換するだけです。
...
これで、\texttt{x*x} と \texttt{z*z} はそれぞれ一度だけ計算されます。
この性質は、ラムダリフティングとグラフ簡約に基づく実装だけでなく、\textbf{言語のあらゆる実装に}当てはまることに注意してください。
これは、完全遅延とラムダリフティングが一見したほど密接に関連していないことを示す手がかりであり、この話題については次のセクションで再び取り上げます。
\vskip\baselineskip

ところで、定義をどの程度先まで浮動させるかは、どうすれば決められるでしょうか？
最も簡単な方法は、\textbf{語彙レベル番号(またはde Bruijn番号)}を使用することです。
3つのステップがあります。

\begin{itemize}
	\item まず、ラムダ式で束縛された各変数にレベル番号を割り当てます。
	      レベル番号は、その変数を囲むラムダの数を表します。
	      したがって、この例では、\texttt{x} にはレベル番号1が割り当てられ、\texttt{y} にはレベル番号2が割り当てられます。
	\item ここで、\texttt{let(rec)} で束縛された各変数(最も外側の変数)にレベル番号を割り当てます。
	      これは、その変数に含まれる自由変数のレベル番号の最大値、または自由変数がない場合は0になります。
	      この例では、\texttt{p} と \texttt{z} の両方にレベル番号1が割り当てられます。
	      \texttt{letrec} を正しく処理するには、いくつかの点に注意する必要があります。
	\item 最後に、各定義(そのバインダのレベルが $n$ だとします)を外側にコーティングします。
	      つまり、バインダーのレベルが $n + 1$ であるラムダ抽象の外側に出て、レベル $n$ の抽象の内部に留まるようにします。
	      このステップでは、2つの間のどこに定義を配置するかについては、ある程度の自由があります。
\end{itemize}

\texttt{letrec} 内で定義された相互再帰的な定義の集合は、互いに依存しており、
単一の \texttt{letrec} 内に留まらなければならないため、まとめて浮上させる必要があります。
実際には、同じ \texttt{letrec} 内に出現しているにもかかわらず相互再帰的\textbf{でない}場合、
このポリシーは、本来であればさらに外側に浮上できる定義を内部スコープ内に保持することで、遅延評価性を失う可能性があります。
標準的な解決策は、各 \texttt{letrec} 式内の定義に対して\textbf{依存関係解析}を実行し、各定義グループを最小のサブグループに分割することです。
これについては 6.8 節で説明します。
\vskip\baselineskip

最後に、浮上処理の前に名前変更パスを実行する必要があります。
これにより、\texttt{let(rec)} 定義の移動によって束縛が変更されるリスクがなくなります。
例えば、次の式
...
名前の衝突を避けるために、各バインダに一意の名前を付けるだけで済みます。

\subsection{ラムダリフティングなしの完全遅延評価}

一見すると、完全遅延評価性を維持するために \texttt{let(rec)} を外側に浮上させる必要があるという要件は、Hughes が提案した、すでに複雑な完全な遅延ラムダリフティングアルゴリズムをさらに複雑にするだけのように思えます。
しかし、単純な変換により、\texttt{let(rec)} フロートによって完全な遅延評価性を実現でき、ラムダリフティングは元の単純なラムダリフタによって実行されます。
\vskip\baselineskip

変換は次のようになります。
\textbf{\texttt{let(rec)} 定義をフローティングする前に、各 MFE $e$ を式 \texttt{let v =} $e$ \texttt{in v} に置き換えます。}
この変換により、MFE に名前が付けられ、\texttt{let(rec)} フローティング変換からアクセスできるようになります。
これにより、新しい定義をフローティング変換で生成できるようになります。
その後、通常のラムダリフティングを実行できます。
例えば、\texttt{g} の元の定義を考えてみましょう。
...
部分式 \texttt{x*x} は MFE なので、単純な \texttt{let} 式に置き換えられます。
...
最後に、通常のラムダリフティングにより、\texttt{v} は \texttt{\textbackslash y} 抽象化内で自由であることがわかり、結果のプログラムは次のようになります。
...
ここでいくつか注意すべき点があります。
まず、極大自由式の元々の定義は、\textbf{特定の}ラムダ抽象化を基準としていました。
私たちが開発した新しいアルゴリズムは、特定の式を自明な \texttt{let} 式に変換します。
どのような式がこのように変換されるのでしょうか？
変換されるのは、任意のラムダ抽象化の MFE である式だけです。
例えば、次の式では：
...
2つのMFEが識別されます: \texttt{(x*x)} は \texttt{\textbackslash y} 抽象化の MFE であるため、\texttt{(y + (x*x))} は \texttt{\textbackslash z} 抽象化の MFE であるためです。
自明な \texttt{let} 束縛を導入すると、式は次のようになります。

\begin{verbatim}
	\y. \z. (let v1 = y + (let v2 = x*x in v2) in v1) / z
\end{verbatim}

次に、新しく導入された変数 \texttt{v} は一意であるか、または MFE 識別パスの後に式の名前が一意に変更される必要があります。

第三に、プログラムの最終形式では \texttt{v} は一度しか参照されないため、参照を定義の右側に置き換え、定義を削除するのが合理的です。
こうすることで、Hughes のアルゴリズムを用いて得られたプログラムと全く同じものが得られます。
これは単純な変換であり、ここではこれ以上説明しません。
ただし、この特性はラムダを越えて浮上されたすべての \texttt{let} 定義に当てはまることに注意してください。
いずれにせよ、多くのコンパイラバックエンドは、この変換の実行の有無にかかわらず、同じコードを生成します。

\subsection{完全遅延評価ラムダリフタ}

これで、完全に遅延評価されたラムダリフタを定義する準備が整いました。
これは以下の段階に分解できます。

\begin{itemize}
	\item まず、各 \texttt{ELam} コンストラクタとスーパーコンビネータ定義が単一の引数のみをバインドすることを確認する必要があります。
	      これは、完全遅延ラムダリフタが各ラムダを個別に処理する必要があるためです。
	      これは、アルゴリズムの後の段階で引数のリストを扱うことでエンコードすることも可能ですが、このパスのみを変更することで重要な最適化を表現できることがわかりました。
	      \begin{verbatim}
			separateLams :: CoreProgram -> CoreProgram
		  \end{verbatim}
	\item まず、すべてのバインダと式にレベル番号を付けます。
	      レベル番号は、0から始まる自然数で表します。
	      \begin{verbatim}
			type Level = Int
			addLevels :: CoreProgram -> AnnProgram (Name, Level) Level
		  \end{verbatim}
	\item 次に、すべての MFE を識別し、それらを単純な \texttt{let} 式に置き換えます。
	      レベル番号は、すべての部分式には必要なくなり、バインダにのみ必要になります。
	      \begin{verbatim}
			identifyMFEs :: AnnProgram (Name, Level) Level -> Program (Name, Level)
		  \end{verbatim}
	\item 名前変更パスはすべてのバインダを一意にするため、コーティングによって名前取得エラーが発生することはありません。
	      これは、新しいバインディングを導入する \texttt{identifyMFEs} の後に実行する必要があります。
	      残念ながら、これは現在の \texttt{rename} 関数を使用できないことを意味します。
	      これは \texttt{CoreProgram} に対して機能するのに対し、\texttt{identifyMFEs} は \texttt{Program (Name, Level)} を生成するためです。
	      この目的のために、新しい関数 \texttt{renameL} を作成します。
	      \begin{verbatim}
			renameL :: Program (Name, a) -> Program (Name, a)
		  \end{verbatim}
	\item これで、\texttt{let(rec)} 定義を外側にフロートできるようになりました。
	      レベル番号は不要になりました。
	      \begin{verbatim}
			float :: Program (Name,Level) -> CoreProgram
		  \end{verbatim}
	\item 最後に、セクション 6.3.1 の \texttt{lambdaLift} を使用して、通常のラムダ リフティングを実行できます。
\end{itemize}

完全遅延評価されたラムダリフタは、次のパスの組み合わせです。
...
前回と同様に、\texttt{case} 式の定義のほとんどは演習として残しておきます。

\subsection{ラムダの分離}

補助関数 \texttt{separateLams\_e} を用いて \texttt{separateLams} を定義します。
これは、式内のラムダ抽象化でバインドされた変数を再帰的に分離します。
...
さて、トップレベル関数の \texttt{separateLams} に戻りましょう。
興味深いのは、スーパーコンビネータの定義をどうするかということです。
最も簡単なのは、それらを同等のラムダ抽象化に変換することです。
...
式にレベル番号を注釈付けする場合、いくつか複雑な問題があります。
一見すると、レベル番号を注釈付けした式を返す関数を書けば十分のように見えます。
そして、例えば応用として、2つの部分式のレベル数の最大値を単に取れば良いのです。
残念ながら、このアプローチでは多くの情報が失われます。
なぜなら、ラムダ抽象化の\textbf{本体}のレベル番号を抽象化\textbf{自体}のレベル番号にマッピングする方法がないからです。
最も簡単な解決策は、
まず式に自由変数を注釈付けし、
次に変数からレベル番号へのマッピング \texttt{freeSetToLevel} を使用して、自由変数の注釈をレベル番号に変換することです。
...
2つ目の複雑な問題は、\texttt{letrec} 式に関するものです。
新しく導入された変数に割り当てる正しいレベル番号は何でしょうか？
正しい方法は、再帰変数を\textbf{除いた}すべての右辺の自由変数のレベルの最大値を取るか、
あるいは、この最大値を取る際に再帰変数をレベル0にマッピングすることです。
このレベルは、新しい変数それぞれに割り当てる必要があります。
\texttt{let} 式ははるかに単純です。
新しい変数それぞれに、その右辺のレベル番号を割り当てるだけです。
\vskip\baselineskip

これで \texttt{addLevels} を定義する準備ができました。
これは2つのパスで構成されており、最初のパスでは式に自由変数を注釈付けし、2番目のパスではこの情報を使用してレベル番号の注釈を生成します。

\begin{verbatim}
	addLevels = freeToLevel . freeVars
\end{verbatim}

\texttt{freeVars} 関数は既に定義済みなので、\texttt{freeToLevel} を定義する必要があります。
メイン関数は現在のレベルと、変数からレベル番号へのマッピングを保持する必要があるため、
通常通り、すべての処理を行う \texttt{freeToLevel\_e} を使って \texttt{freeToLevel} を定義します。
...
名前とレベルのマッピングは、型 \texttt{ASSOC Name Level} を持つ連想リストとして表現します。
連想リストのインターフェースは Appendix A に記載されていますが、抽象的\textbf{ではない}ことに注意してください。
連想リスト用の類似物を新たに作成するよりも、リストに関する標準的な関数やリスト表記法をすべて使用する方が便利なので、抽象化を犠牲にしています。
\vskip\baselineskip

これで、各スーパーコンビネータ定義を処理するための補助関数を使用して、\texttt{freeToLevel} を定義できます。
\texttt{separateLams} によってスーパーコンビネータ定義からすべての引数が削除されていることに留意してください。
...
定数、変数、関数適用の場合、自由変数情報を無視してレベル番号を直接計算する方が簡単で効率的です。
...
ラムダ抽象化では同じことはできないため、\texttt{freeSetToLevel} を使って抽象化のレベル番号を計算する必要があります。
また、引数リスト内の各変数にもレベル番号を割り当てます。
現時点では、そのような変数は1つだけであると想定していますが、複数存在できるようにし、それらすべてに同じレベル番号を割り当てます。
これは今のところ正しく動作し、後で有用な最適化(セクション6.7.3)をサポートするためにまさに必要なものであることがわかります。
...
\texttt{let(rec)} 式は、このセクションの冒頭で概説したスキームに従います。
...
\texttt{let(rec)} 式全体のレベルが本体のレベルであることに注意してください。
これは、本体がすべてのバインダを直接的または間接的に参照している限り有効です。
いずれかの定義が未使用の場合、\texttt{letrec} にレベル番号を割り当て、未使用の定義で言及されている変数のスコープ外に配置する可能性があります。
これは簡単に修正できますが、式に冗長な定義が含まれていないと仮定する方が簡単です。
次のセクションで説明する依存関係解析によって、そのような定義は除去されます。
\vskip\baselineskip

\texttt{case} 式は先延ばしされます:
...
最後に、補助関数 \texttt{levelOf} は式からレベルを抽出します。
...
MFE は、式のレベル番号とそのコンテキストのレベルを比較することで簡単に識別できます。
これには、コンテキストのレベルを示す補助パラメータが必要です。
...
\texttt{MFE} $e$ が特定されると、我々の戦略はそれを \texttt{let v =} $e$ \texttt{in v;} という形式の単純な \texttt{let} 式で囲むことですが、
すべての MFE がこのように特別な扱いを受けるに値するわけではありません。
例えば、単一の変数または定数からなる MFE をこのような \texttt{let} 式で囲むのは時間の無駄です。
その他の例は 6.7.3 節で示します。
どの MFE が特別な扱いを受けるに値するかに関するこの知識は、関数 \texttt{notMFECandidate} にエンコードします。
...
\texttt{identifyMFEs\_e} は、式のレベル番号とそのコンテキストのレベル番号を比較することで機能します。
それらが同じ場合、または何らかの理由で式が特別な処理の対象とならない場合は、
\texttt{identifyMFEs\_e1} を使用して部分式に \texttt{identifyMFEs\_e} を適用することを除き、式は変更されません。
それ以外の場合は、\texttt{transformMFE} を使用して適切な変換を実行します。
...
\texttt{identifyMFEs\_e1} は、式のコンポーネントに \texttt{identifyMFEs\_e} を適用します。
...
\texttt{identifyMFEs\_e1} がバインダに遭遇すると、ラムダ抽象化と \texttt{let(rec)} 式の式からわかるように、最初の引数として繰り下げられた「現在の」レベル番号が変更されます。
...
上で述べたように、既存の \texttt{rename} 関数を使ってバインダーを一意にできれば良いのですが、その型が間違っています。
\texttt{rename} のコピーを作成し、いくつかの小さな変更を加えて \texttt{renameL} を作成することは可能ですが、
\texttt{rename} または \texttt{renameL} のいずれかを実行するように特化できる単一の汎用名前変更関数 \texttt{renameGen} を作成する方がはるかに便利です。
\vskip\baselineskip

\texttt{renameGen} の型はどうあるべきでしょうか？
適切な質問は、「\textbf{\texttt{rename} において、各バインダが単純な名前であるという事実をどのように利用したか?}」、
あるいは「\texttt{rename} においてバインダに対してどのような操作を実行したか？」です。
\vskip\baselineskip

実際には、新しいバインダを構築する操作は1つだけです。
\texttt{rename\_e} では、この関数は \texttt{newNames} と呼ばれます。
この関数は、名前の供給と名前のリストを受け取り、空になった名前の供給、新しい名前のリスト、そして古い名前と新しい名前をマッピングする連想リストを返します。

\begin{verbatim}
	newNames :: NameSupply -> [Name] -> (NameSupply, [Name], ASSOC Name Name)
\end{verbatim}

\texttt{renameGen} は、\texttt{Name} 型だけでなくあらゆる種類のバインダに対して動作する必要があるため、
\texttt{new-binders} 関数を追加引数として \texttt{renameGen} に渡す必要があります。
つまり、\texttt{renameGen} の型は次のようになります。
...
バインダの型は型変数 \texttt{*} で表されていることに注意してください。
これは、\texttt{renameGen} がこの型で多相的であるためです。
\texttt{renameGen} を使用すると、\texttt{newNames} を \texttt{renameGen} に \texttt{new-binders} 関数として渡すことで、
元の \texttt{rename} 関数を再定義できます。
...
\texttt{renameL} の方が興味深いです。
そのバインダは \texttt{(Name, Level)} のペアなので、別の \texttt{new-binders} 関数を定義する必要があります。
...
関数 \texttt{newNamesL} は \texttt{newNames} と同じことを行いますが、型が \texttt{(Name, Level)} ペアであるバインダに対して行います。
...
さて、\texttt{renameGen} の記述に移りましょう。
いつものように、追加の管理情報を扱う補助関数 \texttt{renameGen\_e} が必要です。
具体的には、\texttt{rename\_e} と同様に、名前の供給と古い名前から新しい名前へのマッピングを引数として取り、結果の一部として枯渇した供給を返す必要があります。
また、\texttt{new-binders} 関数にも渡す必要があります。
...
\texttt{renameGen\_e} を使うと、\texttt{renameGen} を記述できるようになります。
\texttt{rename} と同様に、renameGen は各スーパーコンビネータ定義にローカル関数 \texttt{rename\_sc} を適用します。
...
\begin{description}
	\item[演習 6.9.] 関数 \texttt{renameGen\_e} を記述してください。
		これは \texttt{rename\_e} と非常によく似ていますが、バインダ操作関数を追加の引数として取る点が異なります。
		\texttt{ELet}、\texttt{ELam}、\texttt{ECase} (それぞれ新しい変数をバインドする)の式では、関数 \texttt{newBinders} を上記の \texttt{rename\_sc} と全く同じように使用できます。
		\vskip\baselineskip

		単純なラムダリフタが \texttt{rename} の新しい定義でも引き続き機能することを確認して、定義をテストします。
	\item[演習 6.10] \texttt{renameL} 用に記述した型シグネチャは、実際には必要以上に制限が厳しくなっています。
		コードを全く変更せずに、これをより汎用的にするにはどうすればよいでしょうか？
		ヒント： \texttt{renameL} は、バインダの2番目のコンポーネントが型レベルであるという事実をどのように利用しているでしょうか?
\end{description}

このセクションでは、高階関数がプログラムのモジュール化にどのように役立つかをわかりやすく説明します。

\subsection{\texttt{let(rec)} 式のフローティング}

最終パスは \texttt{let(rec)} 式を適切なレベルに浮上させます。
式を処理する補助関数は、式と、その式の外側に浮上されるべき定義の集合を返す必要があります。

\begin{verbatim}
	float_e :: Expr (Name, Level) -> (FloatedDefns, Expr Name)
\end{verbatim}

\texttt{floatedDefns} 型にはさまざまな表現方法がありますが、ここでは単純な表現方法を選択します。
つまり、浮上される定義をリストとして表現します。
リストの各要素は定義のグループを表し、そのレベルと \texttt{isRec} フラグによって識別されます。

\begin{verbatim}
	type FloatedDefns = [(Level, IsRec, [(Name, Expr Name)])]
\end{verbatim}

リスト内の定義は相互に依存する可能性があるため、次の制約を追加します。

\begin{quote}
	定義グループは、\texttt{floatedDefns} リスト内の前にある定義グループにのみ依存します。
\end{quote}

これで \texttt{float\_e} の定義に進むことができます。
変数、定数、および適用のケースは単純です。
...
定義はどの程度外側に浮上させるべきでしょうか？
複数の選択肢がありますが、ここでは自由変数の1つを束縛する最も内側のラムダのすぐ内側に定義を設置することを選択します
(6.6.6節で述べたように、単一の \texttt{ELam} 構造によって束縛されるすべての変数には同じレベルが与えられます)。
...
\texttt{let(rec)} 式の等式は、その定義群を、本体から出力された定義群と右辺から出力された定義群に追加します。
新しい定義群は右辺と左辺に依存する可能性があるため、右辺が最初に来なければなりません。
...
補助関数 \texttt{partitionFloats} は、\texttt{floatedDefns} とレベル番号を受け取り、それを外側のレベルに属するものと指定されたレベル(または内側のレベル)に属するものの2つに分割します。
...
関数 \texttt{install} は、指定された定義を含む \texttt{let(rec)} のネストされたセットで式をラップします。
...
最後に、トップレベル関数 \texttt{float} を定義します。
これは \texttt{float\_sc} を使用して \texttt{float\_e} を各スーパーコンビネータに適用し、上記の \texttt{collectSCs} と同じようにスーパーコンビネータのリストを生成します。

\begin{verbatim}
	float prog = concat (map float_sc prog)
\end{verbatim}

関数 \texttt{float\_sc} は、スーパーコンビネータ定義を、元の定義の変換バージョンと、その本体からフロートされたレベル0定義で構成されるスーパーコンビネータ定義のリストに渡します。
...
プログラムのトップレベルは暗黙的に相互再帰的であるため、\texttt{isRec} フラグは省略できます。
また、浮上された各定義はスーパーコンビネータ定義になったため、空の引数リストを与える必要があります。

\newpage

\section{Mark 5: 完全遅延処理の改善}

これで、完全遅延ラムダリフタの定義が完了しました。
出力は常に正しいのですが、必要以上に大きく、効率も低くなります。
このセクションでは、完全遅延変換を改善するいくつかの方法について説明します。

\subsection{\texttt{case} 式の追加}

\begin{description}
	\item[演習 6.11.] \texttt{freeToLevel\_case}、\texttt{identifyMFEs\_case1}、\texttt{float\_case} の定義を記述してください。
		これらはすべてラムダ抽象化と同様の動作をします。
		ヒント: \texttt{float\_case} では、引数リストが空の選択肢に注意してください。
\end{description}

\subsection{冗長なスーパーコンビネータの削除}
...
ここで、\texttt{\textbackslash y} 抽象化には \texttt{x} 自体以外に MFE がないため、完全遅延パスは式に全く影響を与えません。
残念ながら、単純なラムダリフタである \texttt{lambdaLift} は、各ラムダに対して1つずつ、合計2つのスーパーコンビネータを生成しますが、必要なのは1つだけです。
ネストされた \texttt{ELam} 式を1つの \texttt{ELam} に結合してからプログラムを \texttt{lambdaLift} に渡す方が、後者は1つのスーパーコンビネータだけを生成するので、より良いでしょう。
これを別のパスで行うこともできますが、\texttt{float} の処理の一部として行う方が作業が省けます。

\begin{description}
	\item[演習 6.12.] \texttt{float} の \texttt{ELam} ケースの定義を修正し、ネストされた \texttt{ELam} コンストラクタを結合するようにします。
		ヒント: 次の関数を利用します。
		\begin{verbatim}
...
\subsection{冗長な完全遅延評価の回避}

完全な遅延評価はタダではありません。
主に2つの悪影響があります。

\begin{itemize}
	\item \texttt{\textbackslash x y. E} のような複数のラムダ抽象化は、単純なスキームでは1つのスーパーコンビネータになりますが、完全な遅延評価スキームでは2つになる可能性があります。
	      したがって、2つの引数に適用するには1つではなく2つのリダクションが必要になり、コストが高くなる可能性があります。
	\item MFE をリフトアウトすると、部分式がコンテキストから削除され、コンパイラが最適化を実行する機会が減少します。
	      このような最適化は、コンテキストを再度把握するプロシージャ間解析によって部分的に回復される可能性がありますが、問題の発生を回避する方が依然として望ましいと言えます。
\end{itemize}

これらの点は、[Fairbairn 1985] と [Goldberg 1988] によって詳しく説明されています。
さらに、彼らは、\textbf{すべての}ラムダ抽象化から\textbf{すべての} MFE を分離しても、多くの場合メリットは得られないことを指摘しています。
特に：

\begin{itemize}
	\item 複数の抽象化の部分的な適用を共有できない場合、ネストされた抽象化の\textbf{間に} MFE をコーティングしても何も得られません。
	\item 簡約可能でない式であるMFEを取り除いても、得られるものはほとんどありません。
	      作業の共有は行われませんが、クロージャの構築は一度だけで済むため、ストレージ容量をいくらか節約できる可能性があります。
	      しかし、式をコンテキストから削除することでコンパイラの最適化が失われることの方が、このメリットをはるかに上回ります。
\end{itemize}

これらの観察は、完全に遅延評価されたラムダリフタにいくつかの改善を示唆しており、それらは組み込むのが非常に簡単であることがわかりました。

\begin{itemize}
	\item 多重抽象化が \texttt{separateLam} パスによって個別の \texttt{ELam} コンストラクタに分割されない場合、それによって束縛されるすべての変数に同じレベル番号が与えられます。
	      したがって、内側の抽象化では自由だが外側の抽象化では自由でない MFE は識別されません。
	      これにより、単一の \texttt{ELam} コンストラクタによって表される2つの抽象化の間に MFE がコーティングされないことが保証されます。
	      必要なのは、部分適用を共有できない多重抽象化をそれぞれ単一の \texttt{ELam} コンストラクタに保持するように、\texttt{separateLams} パスを変更することだけです。
	      この共有情報は簡単に推測できるものではありませんが、少なくともアルゴリズムのごく一部を変更するだけで、その結果をエレガントに使用できます。
	      これが、\texttt{ELam} コンストラクタがバインダのリストを取ることを許可している理由の1つです。
	\item \texttt{identifyMFEs} は、述語 \texttt{notMFECandidate} を使用して、特定の部分式を MFE として識別するかどうかを決定します。
	      これは、redex ではない式を考慮から除外するための条件を追加するための便利な場所を提供します。
	      この条件も一般には決定不可能ですが、多くの場合、良好な近似値を作成できます。たとえば、\texttt{(+ 3)} は明らかに redex ではありません。
\end{itemize}

これで、完全な遅延評価変換の説明は終了です。
\newpage

\section{Mark 6: 依存関係分析 \dag}
...
内側の \texttt{letrec} は全く再帰的ではありません！
このプログラムは以下のものと同等です。
...
\texttt{let(rec)} ブロックを最小サイズのグループに分割し、\texttt{let} を \texttt{letrec} よりも常に優先するこの変換は、\textbf{依存関係分析}と呼ばれます。
\vskip\baselineskip

プログラムに依存関係分析を施すことで、より良いコードが生成される可能性があることについては既に触れました。
これは以下の2つの場面で現れています。

\begin{itemize}
	\item ジョンソンスタイルのラムダリフティングでは、\texttt{letrec} ブロックの定義における自由変数を単一のエンティティとして扱いました。
	      もし実際に \texttt{letrec} をより小さなブロックに分割できれば、各ブロックに含まれる自由変数の数は少なくなります。
	      これにより、\texttt{abstractJ} 関数によって関数定義に追加しなければならない自由変数の数が削減されます。
	\item 完全遅延変換では、\texttt{let(rec)} ブロック内の宣言を常にまとめて保持していました。
	      まず依存関係解析を行い、宣言を小さなグループに分割すれば、グループの一部を以前よりも外側に展開できる可能性があります。
\end{itemize}

このセクションでは、コア言語プログラムの依存関係分析を行う方法について説明します。

\subsection{強く連結された成分}

依存関係の分析について議論するには、グラフ理論を理解する必要があります。
そこで、まず定義から始めます。

\begin{description}
	\item[定義 6.1] \textbf{有向グラフは2つの要素の組 $(V, E)$ である:}
		\begin{itemize}
			\item 頂点(またはノード)の集合 $V$。
			\item 辺の集合 $E$。
			      ここで各辺はペア $(v_0, v_1)$ であり、$v_0 \in V$ は辺の始点、$v_1 \in V$ は終点です。
		\end{itemize}
\end{description}

次の式では、\texttt{x} は \texttt{y} と \texttt{z} に依存し、\texttt{z} は \texttt{x} と \texttt{y} に依存しますが、\texttt{y} は他の変数には依存しないことがわかります。
...
この定義ブロック用に構築するグラフには、3つの頂点 $\{\texttt{x}, ~ \texttt{y}, ~ \texttt{z}\}$ と、次の辺があります。

\[\{(\texttt{x}, ~ \texttt{y}), ~ (\texttt{x}, ~ \texttt{z}), ~ (\texttt{z}, ~ \texttt{x}), ~ (\texttt{z}, ~ \texttt{y})\}\]

最初の辺の解釈は、\texttt{x} が \texttt{y} に依存し、\texttt{y} からの辺が存在しないということは、\texttt{y} が他の変数に依存しないことを意味します。
グラフを図式化すると、次のようになります。
\vskip\baselineskip

\begin{center}
	\includegraphics*[scale=0.5]{Fig_6-8.png}
\end{center}

同じ頂点のペア間の複数の辺については考慮しないため、代わりに辺に関する情報を写像 $outs$ として定式化することができます。

\begin{description}
	\item[定義 6.2] \[outs ~ v = \{v' \mid (v, ~ v') \in E\}\]
\end{description}

したがって、集合 $outs ~ v$ は、$v$ をソースとするエッジのターゲットとなる頂点の集合です。
例では、$outs ~ \texttt{x} = \{\texttt{y}, ~ \texttt{z}\}, ~ outs ~ \texttt{y} = \{\}, ~ outs ~ \texttt{z} = \{\texttt{x}, ~ \texttt{y}\}$ を見てきました。
\vskip\baselineskip

写像 $outs$ と双対となる同様の写像 $ins$ を構築できます。

\begin{description}
	\item[定義 6.3] \[ins ~ v = \{v' \mid (v, ~ v') \in E\}\]
\end{description}

これは、$v$ を指す辺の始点となる頂点の集合です。
例では、$ins ~ \texttt{x} = \{\texttt{z}\}, ~ ins ~ \texttt{y} = \{\texttt{x}, ~ \texttt{z}\}, ~ ins ~ \texttt{z} = \{\texttt{x}\}$ を見てきました。

\begin{description}
	\item[定義 6.4] \textbf{写像 $r^{\ast}$ は写像 $r$ の推移閉包である。
		これは再帰的に次のように定義できる。}
		\begin{itemize}
			\item $a \in r^{\ast} ~ a$
			\item もし$b \in r^{\ast} ~ a$ならば、$r ~ b \subseteq r^{\ast} ~ a$
		\end{itemize}
\end{description}

集合 $outs^{\ast} a$ は、グラフの辺をたどって頂点 $a$ から到達できるすべての頂点の集合です。
$b \in outs^{\ast} ~ a$ が成り立つとき、$b$ は $a$ から到達可能であるといいます。
集合 $ins^{\ast} ~ a$ は、グラフの辺をたどって頂点 $a$ に到達できるすべての頂点の集合です。
\vskip\baselineskip

実行例を使用すると、$outs^{\ast} ~ x = \{x, ~ y, ~ z\}, ~ outs^{\ast} ~ y = \{y\}, ~ outs^{\ast} ~ z = \{x, ~ y, ~ z\}$ となります。
また、$ins^{\ast} ~ x = \{x, ~ z\}, ~ ins^{\ast} ~ y = \{x, ~ y, ~ z\}, ~ ins^{\ast} ~ z = \{x, z\}$ となります。
\vskip\baselineskip

これで、グラフの強連結成分を定義できるようになりました。
非形式的には、頂点 $a$ と $b$ は、$a$ が $b$ から到達可能であり、かつ $a$ から $b$ が到達可能である場合、常に同じ成分に属します。

\begin{description}
	\item[定義 6.5] \textbf{頂点 $a$ を含むグラフの強連結成分は頂点 $scc ~ a$ の集合であり、次のように定義される。}
		\[scc ~ a = outs^{\ast} ~ a \cap ins^{\ast} a\]
\end{description}

つまり、頂点 $b$ が $scc ~ a$ に属する場合、かつ $a$ から $b$ に到達可能であり、かつ $a$ が $b$ から到達可能である場合に限ります。
実行例のグラフには、2 つの強連結成分 $\{x, ~ z\}$ と $\{y\}$ があります。

\begin{description}
	\item[演習6.13.] 「同じ強連結成分内」という関係が頂点集合を同値類に分割することを証明しなさい。
\end{description}

\subsubsection*{トポロジカルソート}

頂点の順序付けは、写像 $ins^{\ast}$ と $outs^{\ast}$ を考えることで誘導できます。
これは半順序であり、$\preceq$ と表します。

\begin{description}
	\item[定義 6.6] \textbf{頂点 $a$ が位相的に頂点 $b$ 以下 ($a \preceq b$ と表記) となるのは、次の場合のみである:}
		\[b \in outs^{\ast} ~ a\]
\end{description}

この例では、
$\texttt{x} \preceq \texttt{x}, ~ \texttt{x} \preceq \texttt{y}, ~ \texttt{x} \preceq \texttt{z}, ~ \texttt{y} \preceq \texttt{y}, ~ \texttt{z} \preceq \texttt{x}, ~ \texttt{z} \preceq \texttt{y}, ~ \texttt{z} \preceq \texttt{z}$
です。
\vskip\baselineskip

強連結成分は互いに素であるため、それらの要素に対する$\preceq$の作用を考慮することで、同様の順序付けが誘導されます。
実行例では、2つの成分は$\{\texttt{x}, ~ \texttt{z}\} \preceq \{\texttt{y}\}$のように順序付けられています。

\begin{description}
	\item[定義 6.7] \textbf{頂点のシーケンス(または強く連結された成分)は、シーケンス内のすべてのaとbについて、$a \preceq b$の場合に常にaがbに先行する場合、トポロジカルにソートされている。}
\end{description}

したがって、トポロジカルにソートされたシーケンスの1つは \texttt{[x, z, y]} です。
もう1つは \texttt{[z, x, y]} です。
\vskip\baselineskip

2つの強く連結された成分が $\{x, ~ z\} \preceq \{y\}$ の順序になっていることがわかったので、元の式を次のように変換できます。
...
次に、強連結成分アルゴリズムを実装する効率的な方法を検討します。

\subsection{強連結成分アルゴリズムの実装}

\subsubsection*{深さ優先探索}

まず、グラフの深さ優先探索を実装する問題について考えます。
関数 \texttt{depthFirstSearch} は、頂点からその子孫への写像に対してパラメータ化されます。これにより、辺を走査する方向を反転することができます。
さらに、写像 $ins$ と $outs$ を、頂点から頂点のシーケンスへの関数にすることにしました。
この変更の理由は、子孫を何らかの順序で走査する必要があり、集合ではなくシーケンスの方が順序付けが容易だからです。
...
関数 \texttt{depthFirstSearch} は、\texttt{foldll} を使用して入力頂点列を順に探索しながら状態を更新します。
状態は2つの部分で構成されます。
最初の部分では、これまでに訪れたすべての頂点を追跡し、2番目の部分では出力する頂点列を構築します。
...
深さ優先探索の重要な部分は \texttt{search} にコード化されています。
頂点をすでに訪問している場合、状態は変更されません。
...
一方、今回初めて頂点を訪問する場合は、この頂点から探索を進めなければなりません。
この探索から状態が返されたら、現在の頂点をシーケンスに追加する必要があります。
...
検索を開始する前に、訪問したセットを現在の頂点で更新する必要があり、検索する頂点のリストは関数 \texttt{relation} を適用することによって決定されます。
...
結果は、訪問された頂点のセットと、訪問された頂点のシーケンスがトポロジカルなソート順で生成されます。

\begin{description}
	\item[演習 6.14.] 次の定理を証明しなさい。

		\[\texttt{depthFirstSearch} outs ~ (\{\}, ~ []) ~ S = (V, ~ S')\]

		ここで、$S$ は頂点の列、$V = \bigcup_{v \in S} outs ~ v, ~ S'$ は $V$ 内の頂点のトポロジカルにソートされた列です。
\end{description}

\subsubsection*{スパニング検索}

関数 \texttt{spanningSearch} は関数 \texttt{depthFirstSearch} をわずかに改良したもので、検索中に取得された構造化情報を保持します。
...
現在の頂点がすでに訪問されている場合は、現在の状態を返します。
...
あるいは、現在の頂点を初めて訪れる場合は、現在の頂点から(\texttt{depthFirstSearch} を使用して)検索します。
返されるシーケンスは、現在の頂点に関連付けられたコンポーネントを構成します。
したがって、これを構築中のセットのシーケンスに追加します。
...
\subsubsection*{強く連結された成分}

強く連結されたコンポーネントアルゴリズムは、次の関数として実装できるようになりました。
...
関数 \texttt{scc} はグラフ上の2つのパスで構成されます。
...
最初に、グラフの頂点のトポロジカルにソートされたシーケンスを構築します。

\begin{verbatim}
	    where depthFirst = second . depthFirstSearch outs (setEmpty, [])
\end{verbatim}

2番目のパスでは、強く接続されたコンポーネントのトポロジカルにソートされたシーケンスの逆を構築します。

\begin{verbatim}
	          spanning = second . spanningSearch ins (setEmpty, [])
\end{verbatim}

最初の要素を構築するときに何が起こるかを考えてみましょう。
シーケンスの先頭には頂点 $a$ があります。
$a \preceq b$ を満たす他の頂点は、シーケンスの後半に現れます。
$b \preceq a$ を満たす頂点はありません。
関係パラメータとして $ins$ を指定して \texttt{spanningSearch} を呼び出すと、$ins^{\ast} ~ \texttt{a}$ が構築されます。
訪問された集合は、要素内の各頂点で拡張されます。
\vskip\baselineskip

先ほどの例では、リスト $\left[\texttt{x}, ~ \texttt{z}, ~ \texttt{y}\right]$ に \texttt{spanning} を適用します。
これは次のように展開されます。
...
内部の \texttt{search} を拡張すると次のようになります。
...
しかし、$\texttt{ins x} = \left[\texttt{z}\right]$ なので、$\texttt{vs} = \{\texttt{x}, ~ \texttt{z}\}$ かつ $\texttt{s} = \left[\texttt{z}\right]$ となります。
したがって、式は次のように簡約されます。
...
\texttt{z} はすでに訪問済みセットに含まれているため、次のようになります。

\begin{verbatim}
	second (search ins ({x,z}, [{x,z}]) y)
\end{verbatim}

\texttt{search} は次のように拡張されます。

強く接続されたコンポーネントは逆のトポロジ順序で出力されることに注意してください。
...
しかし、$\texttt{ins y} = \left[\texttt{x}, ~ \texttt{z}\right]$ は両方とも既に訪問済みなので、$\texttt{vs} = \{\texttt{x}, ~ \texttt{z}, ~ \texttt{y}\}, ~ \texttt{s} = \left[\right]$ となります。
\vskip\baselineskip
...
\begin{quote}
	\texttt{spanningSearch} の訪問済みセットは、強力に接続されたコンポーネントにすでに割り当てられている頂点を表します。
\end{quote}

入力シーケンス内で、既に訪問済みの集合に存在する頂点に遭遇した場合、その頂点はそれ以上の検討対象から削除されたかのように振舞います。
頂点bが入力シーケンス内でまだ訪問されていない次の頂点であると仮定します。
計算を行うと、

\[\texttt{depthFirstSearch} ~ ins ~ (\{\texttt{b}\} \cup \texttt{visited}; ~ \left[\right]) ~ (ins ~ \texttt{b})\]

これにより、新しい訪問済みセット ($scc ~ \texttt{a} \cup scc ~ \texttt{b}$) と、$scc ~ \texttt{b}$ 内の頂点を要素とするシーケンスが生成されます。
\vskip\baselineskip
...
\begin{description}
	\item[演習 6.15.]
		\[\texttt{scc} ~ ins ~ outs ~ R = S\]
		そして $V = \bigcup \{ins^{\ast} ~ v \mid v \in R\}$ とします。
		次のことを証明してください。
		\begin{itemize}
			\item まず、シーケンス $S$ にはすべての強連結成分が含まれます。
			      つまり、
			      \[\texttt{setFromList} ~ S = \bigcup \{scc ~ v \mid v \in V\}\]
			\item 次に、これらの成分はトポロジカルに逆順です。
			      つまり、\texttt{a} $\preceq$ \texttt{b} の場合、シーケンス $S$ において \texttt{b} は \texttt{a} の前に出現します。
		\end{itemize}
\end{description}

\subsection{依存関係の分析}

これで、プログラムの依存関係解析を実行できます。
\texttt{let(rec)} ブロックに遭遇するたびに、それを強く連結された要素に分割する必要があります。
\texttt{let} の場合、これは簡単です。
依存関係がないため、定義のリストを各定義のシングルトンリストに分割するだけです。
\vskip\baselineskip

依存関係の解析は関数 \texttt{dependency} によって実行されます。
これは、\texttt{freeVars} の以前のパスからの情報を使用して \texttt{let(rec)} を並べ替えます。
この情報は、補助関数 \texttt{depends} によって使用されます。
...
この作業は、\texttt{depends\_e} によって行われます。
\texttt{depends\_e} の唯一の興味深いケースは、\texttt{let(rec)} の場合です。
...
\texttt{letrec} の場合、依存関係グラフを構築し、\texttt{scc} 関数を適用して定義を分割する方法を決定する必要があります。
\texttt{defnGroups} が $\left[d_1, d_2, \ldots, d_n \right]$ (そして \texttt{letrec} を処理している場合)の場合、\texttt{letrec} は次のように変換されます。

\[\texttt{letrec} ~ d_1 ~ \texttt{in letrec} ~ d_2 ~ \texttt{in} ~ \ldots ~ \texttt{letrec} ~ d_n ~ \texttt{in body}\]
...
私たちが関心を持つ変数のセットはバインダーから派生したもので、\texttt{binderSet} と呼ばれます。
...
これから、依存関係グラフの \texttt{edges} を構築できます。
...
したがって、強く連結されたコンポーネントアルゴリズムに必要な関数の \texttt{ins} と \texttt{outs} が得られます。
...
結果として得られるセットのリストは、\texttt{conponents} と呼ばれるリストのリストに変換されます。

\begin{verbatim}
	      components = map setToList (scc ins outs binders)
\end{verbatim}

元の定義 \texttt{defns} 内の各バインダにバインドされている式を検索して、\texttt{defnGroups} を構築します。
...
\begin{description}
	\item[演習 6.16.] 非再帰的な \texttt{let} からの定義群に加えて、\texttt{letrec} から生じる非再帰的な定義が得られることもあります。
		これは、先ほど使用した例の \texttt{y} の場合です。
		\texttt{letrec} ではなく \texttt{let} でこのような束縛を行うには、\texttt{mkDependLet} を再定義してください。
		ヒント: \texttt{mkDependLet} に渡される定義の右辺にある自由変数情報を使用してください。
\end{description}
\newpage

\section{結論}

完全な遅延評価に対する私たちのアプローチを、同様の問題を扱った Bird の非常に優れた論文 [Bird 1987] と比較するのは興味深いことです。
Bird の目的は、初期仕様を連続的に変換することによって、効率的な完全遅延ラムダリフタを形式的に開発することです。
結果として得られるアルゴリズムはかなり複雑で、直接書き出すのは難しいため、形式的な開発の努力は十分に正当化されます。
\vskip\baselineskip

対照的に、私たちはアルゴリズムを多数の非常に単純なフェーズの構成として表現しており、各フェーズは簡単に指定して直接書き出すことができます。
結果として得られるプログラムは、式を何度も走査するため、定数因数の非効率性を伴います。
これは、連続するパスを1つの関数に折りたたんで、中間データ構造を排除することで簡単に削除できます。
...
\section{\texttt{let}と\texttt{letrec}}
