...
\footnote{いつものように、継続の明示的なラベルを使用してコードを作成しますが、
	実際には、新しいラベルの生成を避けるために、\texttt{Code} アドレッシング モードの使用をコンパイルします。}
...
これまでのところ、すべてのローカル変数 (つまり、スーパーコンビネータ引数または \texttt{let(rec)} バインド変数) には、
...
テンプレートインスタンス化マシンとGマシンの場合、解決策は各スーパーコンビネーターを表すためにヒープ内にノードを割り当てることでした。
...
たとえば、Gマシン Mkap と比較した Take 命令の相対コストをどのように確立できるでしょうか?
