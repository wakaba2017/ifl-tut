...
\begin{tabular} {|l|} \hline
	...
	\\
	\begin{tabular}{r c l l}
		$\mathcal{B} \llbracket n \rrbracket ~ \rho ~ cont$                      & $=$                                                                    & $\mathcal{B} \llbracket e_2 \rrbracket ~ \rho ~ (\mathcal{B} \llbracket e_1 \rrbracket ~ \rho ~ (\texttt{Op Add :} ~ cont))$ &                         \\
		                                                                         & \multicolumn {3} {l|} {$\ldots$および他の算術プリミティブの同様の規則}                                                                                                                                                          \\
		$\mathcal{B} \llbracket ~ e_1 \texttt{+} ~ e_2 \rrbracket ~ \rho ~ cont$ & $=$                                                                    & $\texttt{PushV} ~ (\texttt{IntVConst} ~ n) ~ \texttt{:} ~ cont$                                                              & ここで、$n$は数値です。 \\
