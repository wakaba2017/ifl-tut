...
freeToLevel :: AnnProgram Name (Set Name)
               -> AnnProgram (Name, Level) Level
freeToLevel prog
  = map freeToLevel_sc prog

freeToLevel_sc :: (Name, [Name], AnnExpr Name (Set Name))
                  -> (Name, [(Name, Level)], AnnExpr (Name, Level) Level)
freeToLevel_sc (sc_name, [], rhs)
  = (sc_name, [], freeToLevel_e 0 [] rhs)
...
freeToLevel_case level env free e alts
  = (freeSetToLevel env free, ACase e' alts')
    where
      ...
      alts' = map (freeToLevel_alt level env) alts

freeToLevel_alt :: Level
                -> ASSOC Name Level
                -> AnnAlt Name (Set Name)
                -> AnnAlt (Name, Level) Level
freeToLevel_alt level env (tag, args, rhs)
  = (tag, args', rhs')
    where
...
identifyMFEs_e arityEnv ctx (level, ALet is_rec defns body)
  = let
      processDef :: AnnDefn (Name, Level) Level
                    -> (((Name, Level), Expr (Name, Level)), Bool)
...
identifyMFEs_e arityEnv ctx (level, ACase e alts)
  = let
      -- 1. 条件式 e を ctx のまま処理する
      (e', m1) = identifyMFEs_e arityEnv ctx e
      processAlt :: AnnAlt (Name, Level) Level
                    -> (Alter (Name, Level), Bool)
      processAlt (tag, args, rhs)
        = let
            -- 2. 選択肢 alts を ctx のまま処理する
            (rhs', m) = identifyMFEs_e arityEnv ctx rhs
          in
            ((tag, args, rhs'), m)
      (alts', ms2) = unzip (map processAlt alts)
      childHasMFE = or (m1 : ms2)
      expr = ECase e' alts'
    in
      if childHasMFE
      -- 3. 子に MFE があれば、case 全体は MFE 判定しない
      then (expr, True)
      else
        -- 4. 子に MFE がなければ、新しい条件式 e' に対して MFE 判定を行う
        if level < ctx && okAsMFERoot arityEnv e'
        then (ECase (transformMFE level e') alts', True)
        else (expr, False)
...
renameGen_case :: (NameSupply
                  -> [(Name, a)] -> (NameSupply, [(Name, a)], ASSOC Name Name))
                  -> ASSOC Name Name
                  -> NameSupply
                  -> Expr (Name, a)
                  -> [Alter (Name, a)]
...
renameGen_case new_binders env ns e alts
  = (ns_final, ECase e' alts')
    where
      (ns1, e') = renameGen_e new_binders env ns e
      (ns_final, alts') = renameGen_alts new_binders ns1 alts
      renameGen_alts new_binders ns [] = (ns, [])
      renameGen_alts new_binders ns ((tag, args, rhs) : alts)
        = (ns3, (tag, args', rhs') : alts')
          where
            (ns1, args', env') = new_binders ns args
            rhs_env = env' ++ env
            (ns2, rhs') = renameGen_e new_binders rhs_env ns1 rhs
            (ns3, alts') = renameGen_alts new_binders ns2 alts
...
float_sc :: ScDefn (Name, Level) -> CoreProgram
float_sc (name, [], rhs)
  = [(name, [], rhs')] ++ concat (map to_scs fds)
    where
      (fds, rhs') = float_e rhs
      to_scs :: (Level, IsRec, [(Name, Expr Name)])
                -> [ScDefn Name]
      to_scs (level, is_rec , defns)
        = map make_sc defns
      make_sc :: (Name, Expr Name)
                 -> ScDefn Name
      make_sc (name, rhs)
        = (name, [], rhs)
...
float_e (ECase e alts)
  = float_case e alts

float_defn :: FloatedDefns
              -> ((Name, Level), Expr (Name, Level))
              -> (FloatedDefns, (Name, Expr Name))
...
float_case e alts
  = (fd_alts ++ fd_cond, ECase e' alts')
    where
      (fd_cond, e') = float_e e
      (fd_alts, alts') = mapAccuml float_alt [] alts

float_alt :: FloatedDefns
             -> Alter (Name, Level)
             -> (FloatedDefns, Alter Name)
float_alt floatedDefns (tag, args, rhs)
  = (rhsFloatDefns ++ floatedDefns, (tag, args', rhs'))
    where
      args' | length args == 0 = []
            | otherwise = [arg | (arg, level) <- args]
      (rhsFloatDefns, rhs') = float_e rhs
...
partitionFloats this_level fds
  = (filter is_outer_level fds, filter is_this_level fds)
    where
      is_this_level :: (Level, IsRec, [(Name, Expr Name)])
                       -> Bool
      is_this_level (level, is_rec, defns)
        = level >= this_level
      is_outer_level :: (Level, IsRec, [(Name, Expr Name)])
                        -> Bool
      is_outer_level (level, is_rec, defns)
        = level < this_level
...
install defnGroups e
  = foldr installGroup e defnGroups
    where
      installGroup :: (Level, IsRec, [(Name, Expr Name)])
                      -> Expr Name
                      -> Expr Name
      installGroup (level, is_rec, defns) e
        = ELet is_rec defns e

mkELam :: [Name] -> CoreExpr -> CoreExpr
mkELam args (ELam args' body)
  = ELam (args ++ args') body
mkELam args other_body
  = ELam args other_body
...
depends prog
  = [(name, args, depends_e rhs)
    | (name, args, rhs) <- prog
    ]

depends_e :: AnnExpr Name (Set Name) -> CoreExpr
depends_e (free, ANum n)
  = ENum n
depends_e (free, AConstr t a)
  = EConstr t a
depends_e (free, AVar v)
  = EVar v
depends_e (free, AAp e1 e2)
  = EAp (depends_e e1) (depends_e e2)
depends_e (free, ACase body alts)
  = ECase (depends_e body)
          [ (tag, args, depends_e e)
          | (tag, args, e) <- alts
          ]
depends_e (free, ALam ns body)
  = ELam ns (depends_e body)
depends_e (free, ALet is_rec defns body)
  = foldr (mkDependLet is_rec) (depends_e body) defnGroups
    where
      ...
      defnGroups :: [[AnnDefn Name (Set Name)]]
      defnGroups = [ [(n, aLookup defns n (error "defnGroups")) | n <- ns]
                   | ns <- components
                   ]

mkDependLet :: IsRec
               -> [(Name, AnnExpr Name (Set Name))]
               -> Expr Name
               -> Expr Name
mkDependLet is_rec dfs e
  | null dfs
      = error "mkDependLet: empty dfs"
  | length dfs > 1
      -- ここは「必ず再帰的」なので letrec のままでよい
      = ELet is_rec [(n, depends_e annRhs) | (n, annRhs) <- dfs] e
  | otherwise
      = case dfs of
        [(n, (freeSet, rhsExpr))] ->
          let
            -- もともと letrec で、かつ 自己参照しているときだけ再帰にする
            is_rec' = is_rec && setElementOf n freeSet
          in
            ELet is_rec' [(n, depends_e (freeSet, rhsExpr))] e
...
scc ins outs
  = spanning . depthFirst
    where
      -- depthFirst :: [a] -> [a]
      depthFirst = second . depthFirstSearch outs (setEmpty, [])
      -- spanning   :: [a] -> [Set a]
