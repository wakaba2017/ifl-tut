...
type GmDumpItem = (GmCode, GmStack, GmVStack)  -- Mark7で変更
...
unwind state
  = newState (hLookup heap a)
    where
      ...
      heap = getHeap state
      -- Mark7で変更
      newState (NNum n) = putVStack v' (putCode i' (putStack (a : s') (putDump d state)))  -- 遷移規則 (3.22)
        where ((i', s', v') : d) = getDump state
      newState (NAp a1 a2) = putCode [Unwind] (putStack (a1:a:as) state)  -- 遷移規則 (3.11)
      newState (NGlobal n c)
        | length as < n = putVStack v (putCode i (putStack (ak : s) (putDump d state)))  -- 遷移規則 (3.29) Mark7で変更
        | otherwise     = putCode c (putStack as' state)  -- 遷移規則 (3.19)
          where ((i, s, v) : d) = getDump state  -- Mark7で変更
      ...
      -- Mark7で変更
      newState (NConstr t as) = putVStack v' (putCode i' (putStack (a : s') (putDump d state)))  -- 遷移規則 (3.35)
        where ((i', s', v') : d) = getDump state
...
-- Mark4で追加
eval' :: GmState -> GmState  -- 遷移規則 (3.23)→(3.48)
eval' state
  = putVStack [] (putCode [Unwind] (putStack [a] (putDump newDump state)))
    where (a : s) = getStack state
          newDump = (getCode state, s, getVStack state) : (getDump state)
...
shortShowVStack vstack
  = iConcat [iStr "[",
             iInterleave (iStr ", ") (map (iStr . shownum) vstack),
             iStr "]"]
