...
statUpdExectime (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth, maxvstkdepth))
  = (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime', totalheap, totalclosure, maxstkdepth, maxvstkdepth))
    where
...
statUpdAllcdheap (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth, maxvstkdepth))
  = (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap', totalclosure, maxstkdepth, maxvstkdepth))
    where
...
statUpdAllcdclosure (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth, maxvstkdepth))
  = (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure_, maxstkdepth, maxvstkdepth))
    where
...
statUpdMaxstkdpth (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth, maxvstkdepth))
  = (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth', maxvstkdepth))
    where
...
statUpdMaxvstkdpth (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth, maxvstkdepth))
  = (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, (steps, exctime, totalheap, totalclosure, maxstkdepth, maxvstkdepth_))
    where
...
compileR (EConstr t a) env d = (d, [UpdateMarkers a, Take a a, ReturnConstr t])  -- Mark5で追加
...
compileE (t, args, expr) env d
  = (d_, (t, is_moves ++ is_body))
    where
      n = length args
      is_moves = map (\x -> Move (d + x) (Data x)) [1..n]
      (d_, is_body) = compileR expr env_ (d + n)
...
eval state
  = state : rest_states
    where
      ...
      (_, _, _, _, _, _, _, heap, _, _) = step state  -- Mark5で変更 (データフレームポインタを追加)
...
-- Mark5で変更 (データフレームポインタを追加)
timFinal ([], frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, stats) = True
timFinal state                                                                      = False

-- Mark5で変更 (データフレームポインタを追加)
applyToStats :: (TimStats -> TimStats) -> TimState -> TimState
applyToStats stats_fun (instr, frame, dataframe, usedslot, stack, vstack,
                        dump, heap, cstore, stats)
  = (instr, frame, dataframe, usedslot, stack, vstack, dump, heap, cstore, stats_fun stats)

-- Mark5で変更 (データフレームポインタを追加)
step ((Take t n : instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.x)  Mark3で変更
  | (t >= n) && (length stack >= n) = (instr, fptr', dfptr, usdsltnum_, drop n stack, vstack, dump, heap', cstore, stats)
...
step ([Enter am], fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.6, 4.7, 4.8, 4.9)
  = (instr', fptr', dfptr, usdsltnum_, stack, vstack, dump, heap, cstore, stats)
    where (instr',fptr') = amToClosure am fptr heap cstore
          ...
          instr_ = case am of
                   ...
                   _ -> []
step ((Push am:instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.2, 4.3, 4.4, 4.5)
  = (instr, fptr, dfptr, usdsltnum, amToClosure am fptr heap cstore : stack, vstack, dump, heap, cstore, stats)
step ([Return], fptr, dfptr, usdsltnum, [], n : vstack, (fptru, x, stack) : dump, heap, cstore, stats)  -- 遷移規則 (4.16)  Mark4で追加
  = ([Return], fptr, dfptr, usdsltnum, stack, n : vstack, dump, newHeap, cstore, stats)
    where newHeap = fUpdate heap fptru x (intCode, FrameInt n)
step ([Return], fptr, dfptr, usdsltnum, (instr', fptr') : stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.16)  Mark4で番号のみ変更
  = (instr', fptr', dfptr, usdsltnum_, stack, vstack, dump, heap, cstore, stats)
    where usdsltnum_ = case instr' of
                       ...
                       othrs -> if isPushCodeInvolved othrs
                                ...
                                -}
step ((PushV (IntVConst n) : instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.14)
  = (instr, fptr, dfptr, usdsltnum, stack, n : vstack, dump, heap, cstore, stats)
step ((PushV FramePtr : instr), (FrameInt n), dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.12)
  = (instr, (FrameInt n), dfptr, usdsltnum, stack, n : vstack, dump, heap, cstore, stats)
step ((Op Neg : instr), fptr, dfptr, usdsltnum, stack, n : vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.10)
  = (instr, fptr, dfptr, usdsltnum, stack, -n : vstack, dump, heap, cstore, stats)
step ((Op op : instr), fptr, dfptr, usdsltnum, stack, n1 : n2 : vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.10)
  = (instr, fptr, dfptr, usdsltnum, stack, result : vstack, dump, heap, cstore, stats)
    where
      result = case op of
               Add   -> n1 + n2
               Sub   -> n1 - n2
...
step ([Cond i1 i2], fptr, dfptr, usdsltnum, stack, n : vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.13)
  = (i, fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)
    where i | n == 0    = i1
            | otherwise = i2
step ((Move i (Data n) : instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.x)  Mark5で追加する必要ありそう。
  = (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, newHeap, cstore, stats)
    where
...
step ((Move i a : instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.x)  Mark3で追加
  = (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, newHeap, cstore, stats)
    where
      curFrameSize = case fptr of
                     FrameAddr addr -> length f
...
step ((PushMarker x : instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.16)  Mark4で追加
  = (instr, fptr, dfptr, usdsltnum, [], vstack, (fptr, x, stack) : dump, heap, cstore, stats)
step ((UpdateMarkers n : instr), fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.17)  Mark4で追加
  | length stack >= n = (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)
  | otherwise         = ((UpdateMarkers n : instr), fptr, dfptr, usdsltnum, newStack, vstack, newDump, newHeap, cstore, stats)
                        where
                          ...
                          (_, (fptr_ : _), _)          = heap
step ([Switch brchs], fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.18)  Mark5で追加
  = (instr, fptr, dfptr, usdsltnum, stack, vstack_, dump, heap, cstore, stats)
    where
...
step ([ReturnConstr t], fptr, dfptr, usdsltnum, (instr', fptr') : stack, vstack, dump, heap, cstore, stats)  -- 遷移規則 (4.19)  Mark5で追加
  = (instr', fptr', fptr, usdsltnum_, stack, t : vstack, dump, heap, cstore, stats)
    where usdsltnum_ = case instr' of
                       [] -> usdsltnum
                       Take _ _ : _ -> case fptr of
                                       FrameAddr addr -> take n [1..]
                       ...
                       othrs -> if isPushCodeInvolved othrs
                                ...
                                {-
                                  ...
                                  そうでなかったら、getUsedSlotNumber instr_ の戻り値を usdsltnum_ にする。
                                -}
step ([ReturnConstr t], fptr, dfptr, usdsltnum, [], vstack, (fptru, x, stack) : dump, heap, cstore, stats)  -- 遷移規則 (4.20)  Mark5で追加
  = ([ReturnConstr t], fptr, dfptr, usdsltnum, stack, vstack, dump, newHeap, cstore, stats)
...
gc (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)
  = (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, newHeap, cstore, stats)
    where
...
showSCDefns (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)
...
showState (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, heap, cstore, stats)
  = iConcat [
      iStr "Code: ", showInstructions Terse instr, iNewline,
      showFrame heap fptr,
      iStr "A data frame follows.", iNewline,  -- Mark5で追加
...
showStats (instr, fptr, dfptr, usdsltnum, stack, vstack, dump, heap, code, stats)
