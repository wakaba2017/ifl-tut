...
instance Eq Node
  where
    ...
    NConstr a b == NConstr c d = False  -- not needed  -- Mark6で追加
...
compileC (EAp e1 e2) env
  = compileC e2 env ++
    ...
    (subFuncForEAp e1)
    where subFuncForEAp e =
            case e of
            EConstr _  _ -> []
            EAp     e' _ -> subFuncForEAp e'
            _            -> [Mkap]
{-
...
compileE (EConstr t a) env  -- Mark6で追加
  = [Pack t a]
compileE (EAp e1 e2) env  -- Mark6で追加
  = compileC e2 env ++
    compileC e1 (argOffset 1 env) ++
    (subFuncForEAp e1)
    where subFuncForEAp e =
            ...
            EConstr _  _ -> []
            EAp     e' _ -> subFuncForEAp e'
            _            -> [Mkap] ++ [Eval]  -- Eスキームで関数適用を処理する場合、
                                              -- これまでと違って一番下のデフォルトケースには行かなくなるようなので、
                                              -- ここで[Eval]をappendすることにしてみた。
...
b_3_2_3' = "nfib n = if (n<2) 1 " ++
                    "(nfib (n-1) + nfib (n-2)) ;" ++
           "main = nfib 10"  -- このように書き換えたら、正しい値89が得られた。かかったステップは6184ステップ。
